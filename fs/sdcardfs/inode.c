		 ...
		 struct kstat *stat)
{
	...
	struct path lower_path;
	struct dentry *parent;
    /*[wuzehui] m_blocks for temp stat->blocks */
    unsigned long long	m_blocks;
    ...
    // [wuzehui] start: use lower_path caculate the stat
    vfs_getattr(&lower_path,stat);
    m_blocks = stat->blocks;
    // [wuzehui] end

	generic_fillattr(inode, stat);
    /*[wuzehui] m_blocks for temp stat->blocks */
    stat->blocks = m_blocks;
	sdcardfs_put_lower_path(dentry, &lower_path);
	return 0;
}
