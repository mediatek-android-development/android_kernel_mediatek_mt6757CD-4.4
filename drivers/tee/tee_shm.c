...
struct tee_shm *tee_shm_alloc(struct tee_context *ctx, size_t size, u32 flags)
{
	...
	if (flags & TEE_SHM_DMA_BUF) {
        ...
        #else
        shm->dmabuf = dma_buf_export(shm,&tee_shm_dma_buf_ops,shm->size,O_RDWR,NULL);
		if (IS_ERR_OR_NULL(shm->dmabuf)) {
			ret = ERR_CAST(shm->dmabuf);
			goto err_rem;
		}
        #endif
...
struct tee_shm *tee_shm_register_fd(struct tee_context *ctx, int fd)
{
    ...
    #else
    ref->shm.dmabuf =  dma_buf_export(&ref->shm,&tee_shm_dma_buf_ops,ref->shm.size,O_RDWR,NULL);
	if (IS_ERR(ref->shm.dmabuf)) {
		rc = ERR_PTR(-EINVAL);
		goto err;
	}
