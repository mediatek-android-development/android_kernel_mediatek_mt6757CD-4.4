			    ...
			    struct tee_ioctl_param __user *uparams)
{
	...
	for (n = 0; n < num_params; n++) {
		...
		case TEE_IOCTL_PARAM_ATTR_TYPE_MEMREF_INOUT:
			 ...
			 */
			if (0 == ip.u.memref.shm_id)
			{
				params[n].u.memref.shm_offs = 0;
				params[n].u.memref.size = 0;
				params[n].u.memref.shm = NULL;
			}
			else
			{
				    ...
				    void *driver_data)
{
	...
	snprintf(teedev->name, sizeof(teedev->name), "rsee%s%d",
...
struct KRSEE_UUID{
	...
	__u8 clockSeqAndNode[8];
} ;

#define RSEE_PTA_GET_FPID_UUID \
		{ 0x7adea193, 0x7fe7, 0x9cbb, \
			{ 0xb0, 0x65,0x67, 0x83,0x54,0x86,0x2b,0x8b } }

static int rsee_vers_match(struct tee_ioctl_version_data *vers, const void *data)
{
    printk("%s\n",__FUNCTION__);
    return 1;
}

static  void uuid_to_octets(__u8 d[TEE_IOCTL_UUID_LEN], const struct KRSEE_UUID *s)
{
	d[0] = s->timeLow >> 24;
	d[1] = s->timeLow >> 16;
	d[2] = s->timeLow >> 8;
...
int rsee_client_get_fpid(int *vendor_id)
{
    ...
    struct KRSEE_UUID  uuid = RSEE_PTA_GET_FPID_UUID;
   // struct match_dev_data mdata  = {&vers};
    memset(&vers ,0,sizeof(vers));
    memset(&arg ,0,sizeof(arg));
    memset(&invk_arg ,0,sizeof(invk_arg));
    memset(&param ,0,sizeof(struct tee_param )*4);

    //printk("%s tee_client_open_context  before \n",__FUNCTION__);

    ctx = tee_client_open_context (NULL, rsee_vers_match, NULL, &vers);
    if(!ctx)
        return -EINVAL;
    //printk("%s tee_client_open_context =%p\n",__FUNCTION__,(void*)ctx);

    uuid_to_octets (arg.uuid, &uuid);
    arg.num_params = 0;

    ret = tee_client_open_session (ctx, &arg, (struct tee_param *)&param);
    //printk("%s tee_client_open_session =%d,arg.session=%x\n",__FUNCTION__,ret,arg.session);

    //printk("%s tee_client_open_session arg.ret=%x\n",__FUNCTION__,arg.ret);

    if(ret != 0 || arg.ret != 0)
    {
        //printk("%s tee_client_open_session fail ret=%d,arg.ret=%x\n",__FUNCTION__,ret,arg.ret);
        tee_client_close_context (ctx);
        return -EINVAL;
    }
    invk_arg.func = 0x10000;
    invk_arg.num_params = 1;
    invk_arg.session = arg.session;
    param[0].attr = 2; //value output

    ret = tee_client_invoke_func (ctx, &invk_arg, (struct tee_param *)&param);
    //printk("%s tee_client_invoke_func =%d\n",__FUNCTION__,ret);
    if(ret  == 0)
    {
        //printk("%s tee_client_invoke_func get fp vendor id=[%x]\n",__FUNCTION__,param[0].u.value.a);
        *vendor_id = param[0].u.value.a;
        ret = 0;
    }
    else
    {
        printk("%s tee_client_invoke_func get fp vendor id fail ret=[%x]\n",__FUNCTION__,ret);
    }
    ret = tee_client_close_session (ctx, arg.session);
    //printk("%s tee_client_close_session =%d\n",__FUNCTION__,ret);
    tee_client_close_context (ctx);
    return ret;
}
EXPORT_SYMBOL_GPL(rsee_client_get_fpid);
...
#if 0
static int __init test_pta(void)
{
    int vendor_id;
...
arch_initcall(tee_init);
module_exit(tee_exit);
//module_init(test_pta);
