...
unsigned int is_shippingmode_enable = 0;
/* [lidebiao end] */
...
#endif
		pr_err("[force_get_tbat] %d,%d,%d,%d,%d,%d r:%d %d\n",
...
int force_get_tbat(bool update)
{
	...
	if (bat_temperature_val <= BATTERY_TMP_TO_DISABLE_NAFG) {
		...
		bm_err("[force_get_tbat] ntc_disable_nafg %d %d\n",
			bat_temperature_val, DEFAULT_BATTERY_TMP_WHEN_DISABLE_NAFG);
...
void bmd_ctrl_cmd_from_user(void *nl_data, struct fgd_nl_msg_t *ret_msg)
{
	...
	case FG_DAEMON_CMD_SET_KERNEL_SOC:
	{
		...
		bm_err("[fg_res] FG_DAEMON_CMD_SET_KERNEL_SOC = %d %d\n",
			daemon_soc, FG_status.soc);
		if (get_Runin_Test_Mode() == true) {
			battery_main.BAT_CAPACITY = FG_status.soc;
			battery_update(&battery_main);
		}
		/* [lidebiao end] */
	}
	...
	case FG_DAEMON_CMD_SET_KERNEL_UISOC:
	{
		...
		/* [lidebiao start] Disable charging for Runin test */
		bm_err("[fg_res] FG_DAEMON_CMD_SET_KERNEL_UISOC = %d %d\n",
	...
	case FG_DAEMON_CMD_SET_RTC_UI_SOC:
	{
		...
		/* [lidebiao start] Disable charging for Runin test */
		if (get_Runin_Test_Mode() == true) {
			rtc_ui_soc = FG_status.soc;
		}
		battery_meter_ctrl(BATTERY_METER_CMD_SET_RTC_UI_SOC, &rtc_ui_soc);
		bm_err("[fg_res] BATTERY_METER_CMD_SET_RTC_UI_SOC = %d FG_status.ui_soc = %d FG_status.soc = %d\n",
			rtc_ui_soc, FG_status.ui_soc, FG_status.soc);
		/* [lidebiao end] */
	}
	break;
...
/* [lidebiao start] Added to enter shipping_mode */
static ssize_t show_batt_ship(struct device *dev, struct device_attribute *attr, char *buf)
{

	pr_err("[Battery] is_shippingmode_enable = %x\n", is_shippingmode_enable);
	return sprintf(buf, "%x\n", is_shippingmode_enable);
}

static ssize_t store_batt_ship(struct device *dev, struct device_attribute *attr,
				   const char *buf, size_t size)
{
	int ret;

	pr_err("[Battery] store_ShippingMode\n");
	if (buf != NULL && size != 0) {
		pr_err("[Battery] buf is %s and size is %Zu\n", buf, size);
		ret = kstrtouint(buf, 16, &is_shippingmode_enable);
		pr_err("[Battery] store code: is_shippingmode_enable = %d\n", is_shippingmode_enable);
	}
	return size;
}

static DEVICE_ATTR(batt_ship, 0664, show_batt_ship, store_batt_ship);

bool is_set_transport_mode(void)
{
	if (is_shippingmode_enable == 0x7447) {
		pr_err("[Battery] is_set_transport_mode true\n");
		return true;
	}
	pr_err("[Battery] is_set_transport_mode  false\n");
	return false;
}
/* [lidebiao end] */
					...
					const char *buf, size_t size)
{
	...
	{
		bm_err("bad argument, echo [enable] > current_cmd\n");
	}
...
/*****************************/
	...
	/* [lidebiao start] Added to enter shipping_mode */
	ret_device_file = device_create_file(&(dev->dev), &dev_attr_batt_ship);
	/* [lidebiao end] */
	/* [lidebiao start] Modify Ibat for call active */
