...
int gf_parse_dts(struct gf_dev* pdev)
{
    ...
    if(pdev->spi->dev.of_node) {
        ...
        pdev->fp_enable_high = pinctrl_lookup_state(pdev->pinctrl1, "fp_enable_high");
        if (IS_ERR(pdev->fp_enable_high)) {
            ret = PTR_ERR(pdev->fp_enable_high);
            dev_err(&pdev->spi->dev, " Cannot find fp pinctrl fp_enable_high!\n");
            return ret;
        }
        pdev->fp_enable_low = pinctrl_lookup_state(pdev->pinctrl1, "fp_enable_low");
        if (IS_ERR(pdev->fp_enable_low)) {
            ret = PTR_ERR(pdev->fp_enable_low);
