...
/*----------------------------------------------------------------------------------------------*/
/* [liguanxiong start] */
#define SUNWAVE_LIB_VERSION    "20170612"
#define SUNWAVE_DRIVER_VERSION "v1.0<20170615>"
#define SUNWAVE_CHIP_TYPE      "SW9251"
    ...
    struct device_attribute *attr, const char *buf, size_t count)
{
    if (!strcmp("CMD_SPEED_UP_ON", buf)) {
        g_sunwave_sensor->allow_speed_up = true;
    } else {
        g_sunwave_sensor->allow_speed_up = false;
    }

    if (!strcmp("CMD_MODULE_READY", buf)) {
        if (g_sunwave_sensor->finger && g_sunwave_sensor->finger->reset) {
            g_sunwave_sensor->finger->reset(&g_sunwave_sensor->spi);
        }
    } else if (!strcmp("CMD_MODULE_PREPARE", buf)) {
        if (g_sunwave_sensor->finger && g_sunwave_sensor->finger->power_off) {
            g_sunwave_sensor->finger->power_off(&g_sunwave_sensor->spi);
        }
    }
    return count;
}
...
extern int sunwave_workqueue_init(void);
#endif
/* [liguanxiong end]*/
static int __devinit sunwave_dev_probe(struct spi_device* spi)
{
    ...
    if (status != 0) {
        ...
        /* [liguanxiong] release resource before error return */
        status = -ENODEV;
        goto err_read_id;
    }
    ...
    if (sysfs_create_file(kernel_kobj, &dev_attr_fp_info.attr) != 0) {
        sw_err("Failed to create fp_info.\n");
    ...
    return status;
/* [liguanxiong start] remove registered resource before error return */
err_create_sysfs:
