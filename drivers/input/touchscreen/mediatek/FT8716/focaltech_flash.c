...
{
	#include "FT8716_TP908_TIANMA_app.i"
};
...
#ifdef BOOT_UPGRADE_AUTO// auto upgrade fw in boot of different vendor
...
{
	#include "FT8716_Pramboot_V0.3_20160723.i"
};
...
//extern char manufacture_info[12];
/*******************************************************************************
...
void fts_get_upgrade_array(void)
{
    ...
    TPD_ERR(" %s::FTS_REG_ID=%x\n",__func__,chip_id);
    for (i=0; i<sizeof(fts_updateinfo)/sizeof(struct fts_Upgrade_Info); i++)
    {

		TPD_ERR(" fts_updateinfo[%d].CHIP_ID=%x\n", i,fts_updateinfo[i].CHIP_ID);

		if (chip_id==fts_updateinfo[i].CHIP_ID)
        {
            is_get_chip_id = true;
			TPD_ERR("%s_1::get chip_id\n", __func__);
    ...
    if (!is_get_chip_id)
    {
		...
		TPD_ERR("%s reg_val[0] = %x\n", __func__, reg_val[0]);
        if ((reg_val[0] == 0x86)||(reg_val[0] == 0x87))
        {
			chip_id = 0x87;
            TPD_ERR("[Focal][Touch] read chip id ok from ROM");
        }
        else
        {
            TPD_ERR("[Focal][Touch] read chip id fail from ROM");
        }
    }
#endif

	if((chip_id==0x86)||(chip_id==0x87)){
	TPD_ERR("%s_2 get chip_id = %x\n", __func__, chip_id);
	}else{
...
int  fts_5x46_ctpm_fw_upgrade(struct i2c_client * client, u8* pbt_buf, u32 dw_lenth)
{
    ...
    if (i_ret == 0)
    {
        FTS_DEBUG("[FTS_D] hid change to i2c fail ! \n");
    }
    ...
    for (i = 0; i < FTS_UPGRADE_LOOP; i++)
    {
        ...
        if (i_ret < 0)
        {
            FTS_DEBUG("[FTS_D] failed writing  0x55 and 0xaa ! \n");
            continue;
        }
        /*********Step 3:check READ-ID***********************/
        msleep(1);
        ...
        if (reg_val[0] == fts_updateinfo_curr.upgrade_id_1
            && reg_val[1] == fts_updateinfo_curr.upgrade_id_2)
        {
            FTS_DEBUG("[FTS_D] Step 3: READ OK CTPM ID,ID1 = 0x%x,ID2 = 0x%x\n",reg_val[0], reg_val[1]);
            break;
        }
        else
        {
    ...
    if (i >= FTS_UPGRADE_LOOP )
        return -EIO;
    /*Step 4:erase app and panel paramenter area*/
    FTS_DEBUG("FTS_D Step 4:erase app and panel paramenter area\n");
    auc_i2c_write_buf[0] = FTS_ERASE_APP_REG;
    fts_i2c_write(client, auc_i2c_write_buf, 1);
    msleep(1350);
    for (i = 0; i < 15; i++)
    ...
    FTS_DEBUG("[FTS_D][%s] erase app area reg_val[0] = %x reg_val[1] = %x \n", __func__, reg_val[0], reg_val[1]);
    auc_i2c_write_buf[0] = 0xB0;
    auc_i2c_write_buf[1] = (u8) ((dw_lenth >> 16) & 0xFF);
    auc_i2c_write_buf[2] = (u8) ((dw_lenth >> 8) & 0xFF);
    auc_i2c_write_buf[3] = (u8) (dw_lenth & 0xFF);
    ...
    /*********Step 5:write firmware(FW) to ctpm flash*********/
    bt_ecc = 0;
    FTS_DEBUG("FTS_D Step 5:write firmware(FW) to ctpm flash\n");
    temp = 0;
    packet_number = (dw_lenth) / FTS_PACKET_LENGTH;
    packet_buf[0] = FTS_FW_WRITE_CMD;
    packet_buf[1] = 0x00;
    ...
    if ((dw_lenth) % FTS_PACKET_LENGTH > 0)
    {
        ...
        for (i = 0; i < 30; i++)
        {
            ...
            reg_val[0] = reg_val[1] = 0x00;
            fts_i2c_read(client, auc_i2c_write_buf, 1, reg_val, 2);
            FTS_DEBUG("[FTS_D][%s] reg_val[0] = %x reg_val[1] = %x \n", __func__, reg_val[0], reg_val[1]);
            if ((j + 0x1000) == (((reg_val[0]) << 8) | reg_val[1]))
            {
                break;
            }
    ...
    /*********Step 6: read out checksum***********************/
    /*send the opration head */
    FTS_DEBUG("FTS_D Step 6: read out checksum\n");
    auc_i2c_write_buf[0] = 0x64;
    fts_i2c_write(client, auc_i2c_write_buf, 1);
    msleep(300);
    ...
    FTS_DEBUG(KERN_WARNING "checksum %X %X \n",reg_val[0],bt_ecc);
    /*********Step 7: reset the new FW***********************/
    FTS_DEBUG("FTS_D Step 7: reset the new FW\n");
    auc_i2c_write_buf[0] = FTS_REG_RESET_FW;
    fts_i2c_write(client, auc_i2c_write_buf, 1);
    msleep(200);
    i_ret = hidi2c_to_stdi2c(client);
...
int  fts_8716_writepram(struct i2c_client * client, u8* pbt_buf, u32 dw_lenth)
{
    ...
    FTS_DEBUG("8716 dw_lenth= %x",dw_lenth);
    if (dw_lenth > 0x10000 || dw_lenth ==0)
    {
        return -EIO;
    }
    ...
    for (i = 0; i < 20; i++)
    {
        ...
        if ((reg_val[0] == 0x87
             && reg_val[1] == 0x16))
        {
            FTS_DEBUG("[FTS] Step 3: READ CTPM ID OK,ID1 = 0x%x,ID2 = 0x%x\n",
                      reg_val[0], reg_val[1]);
            break;
        }
        else
...
int  fts_8716_ctpm_fw_upgrade(struct i2c_client * client, u8* pbt_buf, u32 dw_lenth)
{
    ...
    for (i = 0; i < FTS_UPGRADE_LOOP; i++)
    {
        ...
        fts_i2c_read(client, auc_i2c_write_buf, 4, reg_val, 2);
		TPD_ERR("fts_updateinfo_curr.upgrade_id_1 = %x\n",fts_updateinfo_curr.upgrade_id_1);
		TPD_ERR("fts_updateinfo_curr.upgrade_id_2 = %x\n",fts_updateinfo_curr.upgrade_id_2);
        if ((reg_val[0] == fts_updateinfo_curr.upgrade_id_1
    ...
    if (i >= FTS_UPGRADE_LOOP )
        return -EIO;
    /* Step 4:erase app and panel paramenter area */
    FTS_DEBUG("%s::Step 4:erase app and panel paramenter area\n",__func__);
    ...
    /********* Step 5:write firmware(FW) to ctpm flash *********/
    bt_ecc = 0;
    FTS_DEBUG("%s::Step 5:write firmware(FW) to ctpm flash\n",__func__);
    temp = 0;
    packet_number = (dw_lenth) / FTS_PACKET_LENGTH;
    packet_buf[0] = 0xbf;
	FTS_DEBUG("%s::packet_number=%d\n",__func__,packet_number);
    for (j = 0; j < packet_number; j++)
    {
        temp = 0x1000+j * FTS_PACKET_LENGTH;
        packet_buf[1] = (u8) (temp >> 16);
        ...
        fts_i2c_write(client, packet_buf, FTS_PACKET_LENGTH + 6);
		FTS_DEBUG("==write fw package %d , sum %d \n", j, packet_number);
		//FTS_DEBUG("%s::333333\n",__func__);
    ...
    msleep(50);
    //TPD_ERR("%s::2222222222\n",__func__);

    /********* Step 6: read out checksum ***********************/
    /*send the opration head */
    FTS_DEBUG("%s::Step 6: read out checksum\n",__func__);
    auc_i2c_write_buf[0] = 0x64;
    fts_i2c_write(client, auc_i2c_write_buf, 1);
    msleep(300);
    temp = 0x1000+0;
    ...
    {
        temp = dw_lenth;
        FTS_DEBUG("%s::Step 6_1: read out checksum\n",__func__);
    }
    auc_i2c_write_buf[4] = (u8)(temp >> 8);
    auc_i2c_write_buf[5] = (u8)(temp);
    i_ret = fts_i2c_write(client, auc_i2c_write_buf, 6);
...
int  fts_8607_writepram(struct i2c_client * client, u8* pbt_buf, u32 dw_lenth)
{
    ...
    if (dw_lenth > 0x10000 || dw_lenth ==0)
    {
        FTS_DEBUG("[FTS_UPGRADE]: << %s >> : pramboot is too loog. dw_lenth = 0x%x\n", __func__, dw_lenth);
        return -EIO;
    }
    ...
    for (i = 0; i < 20; i++)
    {
        ...
        if (i_ret == 0)
        {
            FTS_DEBUG("[FTS_UPGRADE]: << %s >> : hid change to i2c fail ! \n", __func__);
        }
        ...
        if (i_ret < 0)
        {
            FTS_DEBUG("[FTS_UPGRADE]: << %s >> : failed writing 0x55 ! \n", __func__);
            continue;
        }
        ...
        {
            FTS_DEBUG("[FTS_UPGRADE]: << %s >> : read rom boot id ok,ID1 = 0x%x,ID2 = 0x%x\n", __func__, reg_val[0], reg_val[1]);
            break;
        }
        else
        {
            dev_err(&client->dev, "[FTS_UPGRADE]: << %s >> : read rom boot id fail,ID1 = 0x%x,ID2 = 0x%x\n", __func__, reg_val[0], reg_val[1]);
            continue;
        }
    }
    ...
    if (i >= FTS_UPGRADE_LOOP )
    {
        FTS_DEBUG("[FTS_UPGRADE]: << %s >> : read rom boot id: %d times, but fail,ID1 = 0x%x,ID2 = 0x%x\n", __func__, FTS_UPGRADE_LOOP, reg_val[0], reg_val[1]);
        return -EIO;
    }

    /********* Step 4:write firmware(FW) to ctpm flash *********/
    bt_ecc = 0;
    FTS_DEBUG("[FTS_UPGRADE]: << %s >> : write pramboot!!\n", __func__);
    ...
    /********* Step 5: read out checksum ***********************/
    /* send the opration head */
    FTS_DEBUG("[FTS_UPGRADE]: << %s >> : read out checksum\n", __func__);
    auc_i2c_write_buf[0] = 0xcc;
    msleep(2);
    fts_i2c_read(client, auc_i2c_write_buf, 1, reg_val, 1);
    if (reg_val[0] != bt_ecc)
    {
        FTS_DEBUG("[FTS_UPGRADE]: << %s >> : checksum fail : FW=%02x bt_ecc=%02x\n", __func__, reg_val[0], bt_ecc);
        dev_err(&client->dev, "[FTS_UPGRADE]: << %s >> : ecc error! FW=%02x bt_ecc=%02x\n", __func__, reg_val[0], bt_ecc);
        return -EIO;
    }
    printk(KERN_WARNING "[FTS_UPGRADE]: << %s >> : checksum %X %X \n", __func__, reg_val[0], bt_ecc);
    msleep(100);

    /********* Step 6: start app ***********************/
    FTS_DEBUG("[FTS_UPGRADE]: << %s >> : start app\n", __func__);
    auc_i2c_write_buf[0] = 0x08;
    fts_i2c_write(client, auc_i2c_write_buf, 1);
    msleep(20);
...
int  fts_8607_ctpm_fw_upgrade(struct i2c_client * client, u8* pbt_buf, u32 dw_lenth)
{
    ...
    for (i = 0; i < FTS_UPGRADE_LOOP; i++)
    {
        ...
        if (i_ret < 0)
        {
            FTS_DEBUG("[FTS_UPGRADE]: << %s >> : failed writing  0x55 and 0xaa ! \n", __func__);
            continue;
        }
        ...
        {
            FTS_DEBUG("[FTS_UPGRADE]: << %s >> : read pram boot id ok,ID1 = 0x%x,ID2 = 0x%x\n", __func__, reg_val[0], reg_val[1]);
            break;
        }
        else
        {
            dev_err(&client->dev, "[FTS_UPGRADE]: << %s >> : read pram boot id fail,ID1 = 0x%x,ID2 = 0x%x\n", __func__, reg_val[0], reg_val[1]);
            continue;
        }
    }
    ...
    if (i >= FTS_UPGRADE_LOOP )
    {
        FTS_DEBUG("[FTS_UPGRADE]: << %s >> : read pram boot id fail,ID1 = 0x%x,ID2 = 0x%x\n", __func__, reg_val[0], reg_val[1]);
        return -EIO;
    }
    ...
    FTS_DEBUG("[FTS_UPGRADE]: << %s >> : erase flash\n", __func__);
    auc_i2c_write_buf[0] = 0x61;
    fts_i2c_write(client, auc_i2c_write_buf, 1);
    msleep(800);
    ...
    /********* Step 5:write firmware(FW) to ctpm flash *********/
    bt_ecc = 0;
    FTS_DEBUG("[FTS_UPGRADE]: << %s >> : write app to flash\n", __func__);
    ...
    if ((dw_lenth) % FTS_PACKET_LENGTH > 0)
    {
        ...
        for (i = 0; i < 30; i++)
        {
            ...
            if ((j + 0x20+ 0x1000) == (((reg_val[0]) << 8) | reg_val[1]))
            {
                FTS_DEBUG("[FTS_UPGRADE]: << %s >> : write app to flash ok!!\n", __func__);
                break;
            }
            //msleep(1);
            fts_delay(1000);
    ...
    if (reg_val[0] != bt_ecc)
    {
        FTS_DEBUG("[FTS_UPGRADE]: << %s >> : ecc error: FW=%02x bt_ecc=%02x\n", __func__, reg_val[0], bt_ecc);
        dev_err(&client->dev, "[FTS_UPGRADE]: << %s >> : ecc error: FW=%02x bt_ecc=%02x\n", __func__, reg_val[0], bt_ecc);
        return -EIO;
    }
    printk(KERN_WARNING "checksum %X %X \n",reg_val[0],bt_ecc);
    /********* Step 7: reset the new FW ***********************/
    FTS_DEBUG("[FTS_UPGRADE]: << %s >> : reset the new FW\n", __func__);
    auc_i2c_write_buf[0] = 0x07;
    fts_i2c_write(client, auc_i2c_write_buf, 1);
    msleep(200);
...
int fts_ctpm_get_i_file_ver(void)
{
    ...
    if (ui_sz > 2)
    {
        ...
        {
            FTS_DEBUG("[FTS_UPGRADE]: << %s >> : CHIP_ID = 0x%x, uc_host_fm_ver = 0x%x\n", __func__, fts_updateinfo_curr.CHIP_ID, CTPM_FW[0x10a]);
            return CTPM_FW[0x10a];
        }
        else if (fts_updateinfo_curr.CHIP_ID==0x86)
        {
            FTS_DEBUG("[FTS_UPGRADE]: << %s >> : CHIP_ID = 0x%x, uc_host_fm_ver = 0x%x\n", __func__, fts_updateinfo_curr.CHIP_ID, CTPM_FW[0x10E]);
            return CTPM_FW[0x10E];
        }
        else if (fts_updateinfo_curr.CHIP_ID==0x58)
        {
            FTS_DEBUG("[FTS_UPGRADE]: << %s >> : CHIP_ID = 0x%x, uc_host_fm_ver = 0x%x\n", __func__, fts_updateinfo_curr.CHIP_ID, CTPM_FW[0x1D0A]);
            return CTPM_FW[0x1D0A];
        }
        else
        {
            FTS_DEBUG("[FTS_UPGRADE]: << %s >> : CHIP_ID = 0x%x, uc_host_fm_ver = 0x%x\n", __func__, fts_updateinfo_curr.CHIP_ID, CTPM_FW[ui_sz - 2]);
...
int fts_ctpm_get_i_file_ver_auto(u8 vendor_id)
{
    ...
    if (vendor_id == FTS_VENDOR_ID_TIANMA)
    {
        ui_sz = sizeof(CTPM_FW_TIANMA);
        if(ui_sz == 0){
			TPD_ERR("fts_ctpm_get_i_file_ver_auto error\n");
            return 0x00;
        }
        if (fts_updateinfo_curr.CHIP_ID==FTS_CHIP_ID){
			TPD_ERR("....equal...\n");
            return CTPM_FW_TIANMA[0x10e];
        }
        else{
			TPD_ERR("....no equal...\n");
            return CTPM_FW_TIANMA[ui_sz - 2];
        }
    }
...
int fts_ctpm_fw_upgrade_with_i_file_auto(struct i2c_client *client,u8 vendor_id)
{
    ...
    if (vendor_id == FTS_VENDOR_ID_TIANMA)
    {
	fw_len = sizeof(CTPM_FW_TIANMA);
        pbt_buf = CTPM_FW_TIANMA;
    }

    if(fw_len == 0){
        TPD_ERR("%s:firmware fw_len is 0\n",__func__);
        return -1;
    }
    /*judge the fw that will be upgraded
    * if illegal, then stop upgrade and return.
    */
    TPD_ERR("%s:TP CHIP_ID is %02X\n",__func__, fts_updateinfo_curr.CHIP_ID);
    if ((fts_updateinfo_curr.CHIP_ID==0x11) ||(fts_updateinfo_curr.CHIP_ID==0x12) ||(fts_updateinfo_curr.CHIP_ID==0x13) ||(fts_updateinfo_curr.CHIP_ID==0x14)
        ||(fts_updateinfo_curr.CHIP_ID==0x55) ||(fts_updateinfo_curr.CHIP_ID==0x06) ||(fts_updateinfo_curr.CHIP_ID==0x0a) ||(fts_updateinfo_curr.CHIP_ID==0x08))
    {
        if (fw_len < 8 || fw_len > 32 * 1024)
    ...
    else if ((fts_updateinfo_curr.CHIP_ID==0x86))
    {
        ...
        i_ret =  fts_8607_ctpm_fw_upgrade(client, pbt_buf, fw_len);
        if (i_ret != 0)
        {
            dev_err(&client->dev, "[FTS] upgrade failed. err=%d.\n", i_ret);
        }
    ...
    else if ((fts_updateinfo_curr.CHIP_ID==0x87))
    {
        ...
        //pbt_buf = CTPM_FW;
        /*call the upgrade function*/
		TPD_ERR("tp908 enter this function\n");
        i_ret = fts_8716_writepram(client, aucFW_PRAM_BOOT, sizeof(aucFW_PRAM_BOOT));
        if (i_ret != 0)
        {
            dev_err(&client->dev, "%s:upgrade failed. err.\n",__func__);
...
unsigned char ft5x46_ctpm_InkId_get_from_boot(  struct i2c_client *client )
{
    ...
    {
        msleep(100);
        FTS_DEBUG("[FTS] Step 1:Reset  CTPM\n");
        /*********Step 1:Reset  CTPM *****/
        ...
        /*********Step 2:Enter upgrade mode *****/
        FTS_DEBUG("[FTS] Step 2:Enter upgrade mode \n");
        auc_i2c_write_buf[0] = FTS_UPGRADE_55;
        fts_i2c_write(client, auc_i2c_write_buf, 1);
        msleep(5);
        auc_i2c_write_buf[0] = FTS_UPGRADE_AA;
            ...
            && reg_val[1] != 0)
        {
            ...
            FTS_DEBUG("[FTS] Step 3 ok: CTPM ID,ID1 = 0x%x,ID2 = 0x%x ,0x%x, 0x%x:\n",
    ...
    if (i >= FTS_UPGRADE_LOOP)
    {
        FTS_DEBUG("[FTS] FTS_UPGRADE_LOOP is  i = %d \n", i);
        return -EIO;
    }
    FTS_DEBUG("[FTS] OK: FTS_UPGRADE_LOOP is  i = %d \n", i);
    auc_i2c_write_buf[0] = 0xcd;
    fts_i2c_read(client, auc_i2c_write_buf, 1, reg_val, 1);
    FTS_DEBUG("[FTS]bootloader version = 0x%x\n", reg_val[0]);
    auc_i2c_write_buf[0] = 0x03;
    auc_i2c_write_buf[1] = 0x00;
    for (i = 0; i < FTS_UPGRADE_LOOP; i++)
    {
        ...
        if (i_ret < 0)
        {
            FTS_DEBUG( "[FTS] Step 4: read vendor id from flash error when i2c write, i_ret = %d\n", i_ret);
            continue;
        }
        //fts_i2c_Read(client, auc_i2c_write_buf, 4, reg_val, 2);
        ...
        FTS_DEBUG("%s: REG VAL ID1 = 0x%x,ID2 = 0x%x\n", __func__,reg_val[0],reg_val[1]);
        break;
...
int ft8607_get_rom_pram_id(struct i2c_client *client, unsigned char * id)
{
    unsigned char auc_i2c_write_buf[10];
    int i_ret;
    ...
    auc_i2c_write_buf[0] = 0x90;
    auc_i2c_write_buf[1] = auc_i2c_write_buf[2] = auc_i2c_write_buf[3] =0x00;
    i_ret = fts_i2c_read(client, auc_i2c_write_buf, 4, id, 2);

    FTS_DEBUG("[FTS] Step 3: CTPM ID,ID1 = 0x%x,ID2 = 0x%x\n",id[0], id[1]);
    if (id[0] == fts_updateinfo_curr.upgrade_id_1
        && id[1] != 0)
    {

        FTS_DEBUG("[FTS] Step 3 ok: CTPM ID,ID1 = 0x%x,ID2 = 0x%x ,0x%x, 0x%x:\n",
                  id[0], id[1], fts_updateinfo_curr.upgrade_id_1, fts_updateinfo_curr.upgrade_id_2);
    }
    else
    {
        dev_err(&client->dev, "[FTS] Step 3 fail: CTPM ID,ID1 = 0x%x,ID2 = 0x%x, 0x%x, 0x%x: \n",
...
static unsigned char ft8607_ctpm_VidFWid_get_from_boot(struct i2c_client *client )
{
    unsigned char auc_i2c_write_buf[10];
    unsigned char reg_val[4] = {0};
    unsigned char i = 0;
    ...
    for (i = 0; i < FTS_UPGRADE_LOOP; i++)
    {
        ...
        i_ret = ft8607_get_rom_pram_id(client, reg_val);
        FTS_DEBUG("[FTS] Step 3: CTPM ID,ID1 = 0x%x,ID2 = 0x%x\n",reg_val[0], reg_val[1]);
        if (reg_val[0] == fts_updateinfo_curr.upgrade_id_1 && reg_val[1] == 0x07)
        {
            FTS_DEBUG("[FTS] Step 3 ok: CTPM ID,ID1 = 0x%x,ID2 = 0x%x ,0x%x, 0x%x:\n",
                      reg_val[0], reg_val[1], fts_updateinfo_curr.upgrade_id_1, fts_updateinfo_curr.upgrade_id_2);
            break;
    ...
    if ((i >= FTS_UPGRADE_LOOP) && (reg_val[0] != fts_updateinfo_curr.upgrade_id_1))
    {
        FTS_DEBUG("[FTS] FTS_UPGRADE_LOOP is  i = %d \n", i);
        return -EIO;
    }
    ...
    for (i = 0; i < FTS_UPGRADE_LOOP; i++)
    {
        i_ret = fts_read_reg(client, FTS_REG_VENDOR_ID, &reg_val);
        if (i_ret == 0)
        {
            vid = reg_val;
            FTS_DEBUG("Step 7: reset the new FW\n");
...
int fts_ctpm_auto_upgrade(struct i2c_client *client)
{
    ...
    u8 uc_tp_fm_ver,uc_tp_vendor_id = 0;
    int i_ret;
	unsigned char auc_i2c_write_buf[10];
    ...
    do
    {
        ...
        TPD_ERR("[FTS_UPGRADE]: << %s >> : uc_tp_fm_ver = 0x%x\n", __func__, uc_tp_fm_ver);
        fts_read_reg(client, FTS_REG_VENDOR_ID, &uc_tp_vendor_id);
        TPD_ERR("[FTS_UPGRADE]: << %s >> : uc_tp_vendor_id = 0x%x\n", __func__, uc_tp_vendor_id);
		if(uc_tp_vendor_id == 0xEF){
			auc_i2c_write_buf[0] = 0x07;
			fts_i2c_write(client, auc_i2c_write_buf, 1);
			msleep(200);
			fts_read_reg(client, FTS_REG_VENDOR_ID, &uc_tp_vendor_id);
		...
		if (uc_tp_vendor_id == FTS_VENDOR_ID_TIANMA_ORIG){
            uc_tp_vendor_id = FTS_VENDOR_ID_TIANMA;
			printk("...tp is tiama\n");
		}else if(uc_tp_vendor_id == 0xEF){
			uc_tp_vendor_id = FTS_VENDOR_ID_TIANMA;
		}

        if ((uc_tp_vendor_id == FTS_VENDOR_ID_TIANMA)|| (uc_tp_vendor_id == FTS_VENDOR_ID_TRULY))
        {
#ifdef  BOOT_UPGRADE_AUTO
            uc_host_fm_ver = fts_ctpm_get_i_file_ver_auto(uc_tp_vendor_id);
            TPD_ERR("[FTS_UPGRADE]: << %s >> : uc_tp_fm_ver = 0x%x, uc_host_fm_ver = 0x%x\n", __func__, uc_tp_fm_ver, uc_host_fm_ver);
			/* [lichenggang start] modify firmware update logic algorithm */
			if (((uc_tp_fm_ver < uc_host_fm_ver) && uc_host_fm_ver != 0) || (uc_tp_fm_ver > 0x8F)) // force upgrade
            {
                bUpgradeFlag = true;
            }
			/* [lichenggang end] */
            if (bUpgradeFlag)
            {
                ...
                if (i_ret == 0)
                {
					...
					if(uc_tp_fm_ver != 0xEF){
						TPD_ERR("...update ok...\n");
						TPD_ERR("[FTS_UPGRADE]: successful! new version=0x%x\n",uc_tp_fm_ver);
					}else{
                    	TPD_ERR("...update fail...update again!\n");
						arch_reset(0,NULL);
					}
                }
                else
                {
					...
					TPD_ERR("[FTS_UPGRADE]:failed!old version=0x%x.\n", uc_tp_fm_ver);
                }
			}
			FT8716_FIRMWARE_VERSION=uc_tp_fm_ver;
#else
            uc_host_fm_ver = fts_ctpm_get_i_file_ver(uc_tp_vendor_id);
            FTS_DEBUG("[FTS_UPGRADE]: << %s >> : uc_tp_fm_ver = 0x%x, uc_host_fm_ver = 0x%x\n", __func__, uc_tp_fm_ver, uc_host_fm_ver);
            if (uc_tp_fm_ver < uc_host_fm_ver || uc_host_fm_ver == 0x01) // force upgrade ver0x01
            {
                bUpgradeFlag = true;
            }
#if (FTS_UPGRADE_PINGPANG_TEST || FTS_UPGRADE_ERROR_TEST)
            ...
            if (bUpgradeFlag)
            {
                ...
                {
                    fts_read_reg(client, FTS_REG_FW_VER, &uc_tp_fm_ver);
                    TPD_ERR("[FTS_UPGRADE]: << %s >> : upgrade to new version 0x%x\n", __func__,uc_tp_fm_ver);
					}
                else
                {
                    FTS_DEBUG("[FTS_UPGRADE]: << %s >> : upgrade failed ret=%d.\n", __func__, i_ret);
                }
            }
#endif
        }
...
unsigned char ft5x46_ctpm_LockDownInfo_get_from_boot(  struct i2c_client *client,char *pProjectCode )
{
    ...
    fts_read_reg(client, FTS_REG_VENDOR_ID, &uc_tp_vendor_id);
    FTS_DEBUG("ft5x46_ctpm_LockDownInfo_get_from_boot, uc_tp_vendor_id=%x\n", uc_tp_vendor_id);
    ...
    if (i >= FTS_UPGRADE_LOOP)
    {
        FTS_DEBUG("ft5x46_ctpm_LockDownInfo_get_from_boot, i=%d\n", i);
        return -EIO;
    }
    /********* Step 4: read project code from app param area ***********************/
    msleep(10);

    FTS_DEBUG("ft5x46_ctpm_LockDownInfo_get_from_boot, msleep(10)\n");
    /* read project code */
    auc_i2c_write_buf[0] = 0x03;
    auc_i2c_write_buf[1] = 0x00;
