...
#define TPD_AUTO_UPGRADE                  //开机升级TP固件
#define BOOT_UPGRADE_AUTO                   //根据不同供应商升级对应固件

//#define Boot_Upgrade_Protect


#define FTS_CHIP_ID                         0x87//FT8716

#define FTS_VENDOR_ID_TIANMA                0x8d//tianma vendor id
#define FTS_VENDOR_ID_TRULY                 0x5a//truly vendor id
#define FTS_VENDOR_ID_TIANMA_ORIG           0x96//this id is old, must upgrade
#define FTS_VENDOR_ID_TRULY_ORIG            0x02//this id is old, must upgrade
...
/*
* Enable stress test
* FTS_UPGRADE_TEST_NUMBER --- stress loop number, our standard is 1000.
*/
#define FTS_UPGRADE_PINGPANG_TEST           0
#define FTS_UPGRADE_TEST_NUMBER             1000
#define FTS_UPGRADE_ERROR_TEST              0

/* Enable will get and display whole upgrade time.  need enable FTS_DEBUG_EN = 1, otherwise no print*/
...
* FTS_DEBUG_EN = 1 --- Enable debug information
*              = 0 --- Disable debug information
*
* PRINT_DEBUG_INFO_METHOD = 0x01 --- printk(): print debug information via printk() function
*                         = 0x02 --- log     : print debug information into log file
*                         = 0x03 --- printk & log : print debug information via printk() function and into log file
* Note: Only when FTS_DEBUG_EN == 1, PRINT_DEBUG_INFO_METHOD will be effective.
*/

/* [lichenggang start] close log */
#define FTS_DEBUG_EN                 0
/* [lichenggang end] */

#define PRINT_DEBUG_INFO_METHOD      0x01
#if((FTS_DEBUG_EN) && (FTS_DEBUG_EN == 1))

#if PRINT_DEBUG_INFO_METHOD == 0x01
#define FTS_DEBUG(fmt, args...)      do{ printk("[FTS]" fmt, ## args); }while(0)
#elif PRINT_DEBUG_INFO_METHOD == 0x02
#define FTS_DEBUG(fmt, args...)      do{ /* log file method not finished yet*/ }while(0)
#elif PRINT_DEBUG_INFO_METHOD == 0x03
#define FTS_DEBUG(fmt, args...)      do{ printk("[FTS]" fmt, ## args); /* log file method not finished yet*/ }while(0)
#endif

#else
#define FTS_DEBUG(fmt, args...)     do{}while(0)
#endif //#if defined(FTS_DEBUG_EN) && (FTS_DEBUG_EN == 1)
#endif
