...
static ssize_t fts_tpfwver_show(struct device *dev, struct device_attribute *attr, char *buf)
{
    ...
    if (fts_read_reg(fts_i2c_client, FTS_REG_FW_VER, &fwver) < 0 || fwver == 255){
...
/************************************************************************
* Name: fts_keyconfig_show
* Brief:  show tp keyconfig
* Input: device, device attribute, char buf
* Output: no
* Return: char number
***********************************************************************/
#define KEY_CONFIG_MAX  32

/************************************************************************
* Name: fts_keyconfig_store
* Brief:  no
* Input: device, device attribute, char buf, char count
* Output: no
* Return: tp_key_config
***********************************************************************/

/************************************************************************
* Name: fts_fw_info_show
* Brief:  show tp fw vwersion
* Input: device, device attribute, char buf
* Output: no
* Return: char number
***********************************************************************/
//#define PANEL_DRIVER_VERSION "1.0"
#define PANEL_INFO_MAX      64
...
static ssize_t fts_tprwreg_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
    ...
    if (2 == num_read_chars)
    {
        /*read register*/
        regaddr = wmreg;
        FTS_DEBUG("[focal](0x%02x)\n", regaddr);
        if (fts_read_reg(client, regaddr, &regvalue) < 0)
            FTS_DEBUG("[Focal] %s : Could not read the register(0x%02x)\n", __func__, regaddr);
        else
            FTS_DEBUG("[Focal] %s : the register(0x%02x) is 0x%02x\n", __func__, regaddr, regvalue);
    }
    else
    {
        regaddr = wmreg>>8;
...
/************************************************************************
* Name: fts_fwupgradeapp_write_func
* Brief:  upgrade from app.bin
***********************************************************************/
static u8 FIRMWARE_VERSION_FROM_BINFILE;
ssize_t firmware_update_write_func(struct file *file, const char __user *buf, size_t count, loff_t *ppos)
{
    char fwname[128];
    //u8 uc_tp_fm_ver=0;
    //struct i2c_client *client = container_of(dev, struct i2c_client, dev);
    ...
    fts_ctpm_fw_upgrade_with_app_file(fts_i2c_client, fwname);
    fts_read_reg(fts_i2c_client, 0xA6, &FIRMWARE_VERSION_FROM_BINFILE);
    pr_err("uc_tp_fm_ver=0x%x\n",FIRMWARE_VERSION_FROM_BINFILE);
...
/************************************************************************
* Name: fts_fwupgradeapp_read_func
* Brief:  upgrade from app.bin
***********************************************************************/
ssize_t firmware_update_read_func(struct file *file, char __user *user_buf, size_t count, loff_t *ppos)
{
    int len = 0;
    int ret = 0;
    char page[512];
    TPD_ERR("button_reverse is:0x%x\n",FIRMWARE_VERSION_FROM_BINFILE);
...
{
    int ret = 0;
    //FTS_DEBUG("tony_test:focal_i2c_read,writebuf=%s,writelen=%d,addr=0x%x,readbuf=%s.readlen=%d\n",
...
{
    int ret = 0;
    //FTS_DEBUG("tony_test:focal_i2c_Write,writebuf=%s,writelen=%d,addr=0x%x\n",
...
static int ftxxxx_ReadInIData(char *config_name, char *config_buf)
{
    ...
    loff_t pos = 0;
    mm_segment_t old_fs;
    FTS_DEBUG("tony_test [ft5x0x_ReadInIData] config_name=%s\n",config_name);
    memset(filepath, 0, sizeof(filepath));
    sprintf(filepath, "%s%s", FTXXXX_INI_FILEPATH, config_name);
    if (NULL == pfile)
        pfile = filp_open(filepath, O_RDONLY, 0);
    ...
    filp_close(pfile, NULL);
    set_fs(old_fs);
    FTS_DEBUG("tony_test [ft5x0x_ReadInIData] isize=%d\n",(int)fsize);
    return 0;
}
                                       ...
                                       struct device_attribute *attr, char *buf)
{
    ...
    //struct i2c_client *client = container_of(dev, struct i2c_client, dev);  jacob use globle fts_wq_data data
    mutex_lock(&fts_input_dev->mutex);
    FTS_DEBUG("tony_test:ft5x0x_ftsvendorid_show:addr=0x%x,FTS_REG_VENDOR_ID=0x%02x\n",
...
{
    //&dev_attr_ftskeyconfig.attr,
    //&dev_attr_ftsfwinfo.attr,
