...
#define UBL_I2C_ADDR 0x20			//change address to 0x20
...
/*        ret = request_irq(touch_irq, (irq_handler_t) tpd_eint_handler, IRQF_TRIGGER_LOW,		//change to trigger_falling
                "TOUCH_PANEL-eint", NULL);*/
        if (ret > 0) {
...
#endif
{
    ...
    printk("synaptics_rmi4_probe start\n");
...
static void synaptics_rmi4_suspend(struct device *dev)
{
    ...
    if (rmi4_data->enable_wakeup_gesture) {
		printk("enter wakeup gesture setting!!!");
...
{
    printk("synaptics tpd_local_init\n");
