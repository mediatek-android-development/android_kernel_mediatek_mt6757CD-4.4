...
BOOLEAN rlmFillScanMsg(P_ADAPTER_T prAdapter, P_MSG_SCN_SCAN_REQ prMsg)
{
	...
	//[lixiangkui start] Fix #33957: the phone reboot when 11k Radio Measure
	//Request Actiong Frame is received. In this case, prCurrReq->ucLength is
	//4, and u2RemainLen will be assigned with a negtive int and become a very
	//large positive int, so IE_SIZE(pucSubIE) will access violation and cause
	//a reboot.
	if (prCurrReq->ucLength <= (3 + OFFSET_OF(RM_BCN_REQ_T, aucSubElements))) {
		printk("rlmFillScanMsg, prCurrReq->ucLength: %d, OFFSET_OF(RM_BCN_REQ_T, aucSubElements): %lu,"
			"prCurrReq->ucLength will be <= 0, return FASLE!\n",
			prCurrReq->ucLength, OFFSET_OF(RM_BCN_REQ_T, aucSubElements));
		return FALSE;
	}
	//[lixiangkui end]
