...
static MINT32 gDveCnt;
static MINT32 gWfmeCnt;
/*******************************************************************************
*
********************************************************************************/
typedef struct {
...
static MBOOL ConfigDVEFrameByReqIdx(MINT32 ReqIdx)
{
#ifdef DPE_USE_GCE
	MUINT32 j;
	unsigned long flags; /* old: MUINT32 flags;*//* FIX to avoid build warning */
	...
	return MTRUE;
#else
	LOG_ERR("ConfigDVEFrameByReqIdx don't support this mode.!!\n");
	return MFALSE;
#endif
}
...
static MBOOL ConfigWMFEFrameByReqIdx(MINT32 ReqIdx)
{
#ifdef DPE_USE_GCE
	MUINT32 j;
	unsigned long flags; /* old: MUINT32 flags;*//* FIX to avoid build warning */
...
static long DPE_ioctl(struct file *pFile, unsigned int Cmd, unsigned long Param)
{
	...
	MBOOL bWMFEFound = MFALSE;
	MBOOL bDveFound = MFALSE;
	int idx;
...
static void DPE_ScheduleDveWork(struct work_struct *data)
{
	...
	if (bResulst == MTRUE) {
		...
		if (gDveCnt > 0) {
			...
			if (DveWriteIdx < 0)
				DveWriteIdx = DveWriteIdx + _SUPPORT_MAX_DPE_FRAME_REQUEST_;
			...
			if (g_DVE_RequestRing.DVEReq_Struct[DveWriteIdx].RequestState == DPE_REQUEST_STATE_PENDING) {
				...
				LOG_ERR("DPE_ScheduleDveWork  state machine error!!, DveWriteIdx:%d, RequestState:%d\n",
				DveWriteIdx, g_DVE_RequestRing.DVEReq_Struct[DveWriteIdx].RequestState);
			}
		}
	}
...
static void DPE_ScheduleWmfeWork(struct work_struct *data)
{
	...
	if (bResulst == MTRUE) {
		...
		if (gWfmeCnt > 0) {
			...
			if (WmfeWriteIdx < 0)
				WmfeWriteIdx = WmfeWriteIdx + _SUPPORT_MAX_DPE_FRAME_REQUEST_;
			...
			if (g_WMFE_ReqRing.WMFEReq_Struct[WmfeWriteIdx].RequestState == DPE_REQUEST_STATE_PENDING) {
				...
				LOG_ERR("ScheduleWmfeWork  state machine error!!, WmfeWriteIdx:%d, RequestState:%d\n",
				WmfeWriteIdx, g_WMFE_ReqRing.WMFEReq_Struct[WmfeWriteIdx].RequestState);
			}
		}
	}
