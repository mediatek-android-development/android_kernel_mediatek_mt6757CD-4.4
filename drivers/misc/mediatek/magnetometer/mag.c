...
static void mag_work_func(struct work_struct *work)
{
	...
	for (i = 0; i < MAX_M_V_SENSOR; i++) {
		...
		if (ID_M_V_MAGNETIC == i) {
			...
			/* [yanlin start] Correct time slot */
			...
			while ((cur_ns - m_pre_ns) >= delay_ms*1800000LL) {
				...
				MAG_ERR("--- m_pre_ns[%ld] behind cur_ns[%ld] exceed delay_ms[%ld] ---\n",
						(long)m_pre_ns, (long)cur_ns, (long)delay_ms);
				m_pre_ns = cur_ns - delay_ms*1000000LL;
		...
		if (ID_M_V_ORIENTATION == i) {
			...
			while ((cur_ns - o_pre_ns) >= delay_ms*1800000LL) {
				...
				MAG_ERR("--- o_pre_ns[%ld] behind cur_ns[%ld] exceed delay_ms[%ld] ---\n",
						(long)o_pre_ns, (long)cur_ns, (long)delay_ms);
				o_pre_ns = cur_ns - delay_ms*1000000LL;
...
static int mag_remove(void)
{
	...
	misc_deregister(&mag_context_obj->mdev);
