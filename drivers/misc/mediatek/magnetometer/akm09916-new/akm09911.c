...
/* [yanlin start] Config akm09916 & 09918 */
#define AKM_DRIVER_ALL_IN_ONE
static char akm_device_name[20] = "unknown";
/* [yanlin end] */
...
static int AKECS_CheckDevice(void)
{
	...
	/* [yanlin start] Check device id */
	buffer[0] = AK09911_REG_WIA2;
	ret = AKI2C_RxData(buffer, 1);
	MAGN_ERR(" AKM check device id = 0x%x, ret = %d\n", buffer[0], ret);
	if (ret < 0) {
		MAGN_ERR("Read AKM device id error!\n");
		return ret;
	}
	if (buffer[0] == 0x0C) {
		MAGN_ERR("Device is AK09918\n");
		strcpy(akm_device_name, "AK09918");
	} else if (buffer[0] == 0x09) {
		MAGN_ERR("Device is AK09916\n");
		strcpy(akm_device_name, "AK09916");
	} else if (buffer[0] == 0x10) {
		MAGN_ERR("Device is AK09915\n");
		strcpy(akm_device_name, "AK09915");
	} else if (buffer[0] == 0x05) {
		MAGN_ERR("Device is AK09911\n");
		strcpy(akm_device_name, "AK09911");
	} else {
		MAGN_ERR("Device is unknown\n");
		return -ENXIO;
	}
#ifndef AKM_DRIVER_ALL_IN_ONE
	if (buffer[0] != 0x09) {
		MAGN_ERR("Device id mismatch!\n");
		return -ENXIO;
	}
#endif
	/* [yanlin end] */
...
static long AKECS_GetData(char *rbuf, int size)
{
	...
	ret = AKI2C_RxData(&rbuf[1], SENSOR_DATA_SIZE -1); // [yanlin] Changed back from 2 to 1
	#else
	rbuf[1]= AK09911_REG_HXL;
...
/* [liliwen edit] Add mag board info (1/3) */
static ssize_t show_board_info(struct device_driver *ddri, char *buf)
{
	return sprintf(buf,"type:\t%s\nvendor:\t%s\ndriver_version:\t%s\n",
		"AKM09916", "AKM", DRIVER_VERSION);
}

/* [yanlin start] for hw info in /sys/kernel */
static ssize_t show_mag_info(struct device *dev, struct device_attribute *attr, char *buf)
{
	return sprintf(buf,"type:\t%s\nvendor:\t%s\ndriver_version:\t%s\n",
		akm_device_name, "AKM", DRIVER_VERSION);
}
static DEVICE_ATTR(mag_info, S_IWUSR | S_IRUGO, show_mag_info, NULL);
/* [yanlin end] */
...
/* [liliwen edit] Add mag board info (2/3) */
...
static struct driver_attribute *akm09911_attr_list[] = {
	...
	&driver_attr_power,
	&driver_attr_regmap,
	/* [liliwen edit] Add mag board info (3/3) */
...
static int akm09911_create_attr(struct device_driver *driver)
{
	...
	err = sysfs_create_file(kernel_kobj, &dev_attr_mag_info.attr);
...
static int akm09911_delete_attr(struct device_driver *driver)
{
	...
	/* [yanlin add] for hw info in /sys/kernel */
	sysfs_remove_file(kernel_kobj, &dev_attr_mag_info.attr);
...
static long akm09911_unlocked_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
  ...
  /* MAG_ERR("akm09911 cmd:0x%x\n", cmd); */
	...
	case ECS_IOCTL_GET_CONF:
		...
		/* [yanlin start] Porting AKM09916 & 09918 */
		#ifdef AKM_Device_AK09916
...
static int akm09911_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	/* [liliwen add] change i2c addr in driver */
	client->addr = AKM09911_I2C_ADDRESS;
...
{
	/* [yanlin start] modified for akm */
#ifdef AKM_Device_AK09916
	const char *name = "mediatek,akm09916";
#else
	const char *name = "mediatek,akm09911";
#endif
	/* [yanlin end] modified for akm */
