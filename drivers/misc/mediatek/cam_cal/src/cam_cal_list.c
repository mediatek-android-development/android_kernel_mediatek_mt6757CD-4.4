...
#define MTK_MAX_CID_NUM 4 /* [huangxiancong] custom for tplink otp */
unsigned int mtkCidList[MTK_MAX_CID_NUM] = {
	0x00000001,/* [huangxiancong] custom for tplink otp */
...
unsigned int cam_cal_check_mtk_cid(struct i2c_client *client, cam_cal_cmd_func readCamCalData)
{
	...
	if (readCamCalData != NULL) {
		readCamCalData(client, 0, (unsigned char *)&calibrationID, 1); /* [huangxiancong] custom for tplink otp */
