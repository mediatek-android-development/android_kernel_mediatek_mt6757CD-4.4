...
#include "lsm6ds3-acc.h"

#define POWER_NONE_MACRO MT65XX_POWER_NONE

#define LSM6DS3_STEP_COUNTER
...
#if defined(LSM6DS3_TILT_FUNC) || defined(LSM6DS3_SIGNIFICANT_MOTION) || defined(LSM6DS3_STEP_DETECTOR)
...
struct lsm6ds3_i2c_data {
...
#if defined(CUST_EINT_LSM6DS3_TYPE)
    ...
    /*data*/
	s16                     offset[LSM6DS3_ACC_AXES_NUM+1];  /*+1: for 4-byte alignment*/
...
static bool smd_enable_status = false;
static bool step_d_enable_status = false;
...
/*static struct data_resolution lsm6ds3_acc_data_resolution[1] = {
	{{0, 12}, 8192},	// +/-4G range
};*/
/*----------------------------------------------------------------------------*/
//static struct data_resolution lsm6ds3_acc_offset_resolution = {{0, 12}, 8192};
...
static int LSM6DS3_enable_tilt(struct i2c_client *client, bool enable)
{
	...
	else
	{
		...
		if(!enable_status && !pedo_enable_status && !smd_enable_status && !step_d_enable_status)
		{
...
/*[wanglei start] Move LSM6DS3_acc_Enable_Func*/
#if 1
static int LSM6DS3_acc_Enable_Func(struct i2c_client *client, LSM6DS3_ACC_GYRO_FUNC_EN_t newValue)
{
	u8 databuf[2] = {0};
...
static int LSM6DS3_Enable_SigMotion_Func_On_Int(struct i2c_client *client, bool enable)
{
	...
	if(enable)
	{
		databuf[0] &= ~LSM6DS3_ACC_GYRO_INT_STEP_D_MASK;//clear 
		databuf[0] |= LSM6DS3_ACC_GYRO_INT_STEP_D_ENABLED;			
	}
	else
	{
		databuf[0] &= ~LSM6DS3_ACC_GYRO_INT_STEP_D_MASK;//clear 
		databuf[0] |= LSM6DS3_ACC_GYRO_INT_STEP_D_DISABLED;		
	}
...
#ifdef LSM6DS3_STEP_DETECTOR
static int LSM6DS3_Enable_Step_Detector_On_Int(struct i2c_client *client, bool enable)
{
	u8 databuf[2] = {0}; 
	int res = 0;
	u8 op_reg = 0;
...
/* [liguanxiong] enable LSM6DS3_Reset_Pedo_Data function  */
#if 1
static int LSM6DS3_Reset_Pedo_Data(struct i2c_client *client, LSM6DS3_ACC_GYRO_PEDO_RST_STEP_t newValue)
{
	u8 databuf[2] = {0}; 
...
{
	u8 databuf[2] = {0}; 
	/* [liguanxiong] Remove for too many log */
	//GSE_FUN();
...
/* [liliwen edit] Add acc board info (1/3) */
#define DRIVER_VERSION    "1.0.0"
static ssize_t show_board_info(struct device_driver *ddri, char *buf)
{
	return sprintf(buf,"type:\t%s\nvendor:\t%s\ndriver_version:\t%s\n",
		"LSM6DS3_ACC", "ST", DRIVER_VERSION);
}

/* [yanlin start] for hw info in /sys/kernel */
#define DRIVER_VERSION    "1.0.0"
static ssize_t show_acc_info(struct device *dev, struct device_attribute *attr, char *buf)
{
	return sprintf(buf,"type:\t%s\nvendor:\t%s\ndriver_version:\t%s\n",
		"LSM6DS3_ACC", "ST", DRIVER_VERSION);
}
static DEVICE_ATTR(acc_info, S_IWUSR | S_IRUGO, show_acc_info, NULL);
/* [yanlin end] */
...
/* [liliwen edit] Add acc board info (2/3) */
...
static struct driver_attribute *LSM6DS3_attr_list[] = {
	...
	&driver_attr_layout,
	/* [liliwen edit] Add acc board info (3/3) */
...
static int lsm6ds3_create_attr(struct device_driver *driver) 
{
	...
	err = sysfs_create_file(kernel_kobj, &dev_attr_acc_info.attr);
...
static int lsm6ds3_delete_attr(struct device_driver *driver)
{
	...
	/* [yanlin add] for hw info in /sys/kernel */
	sysfs_remove_file(kernel_kobj, &dev_attr_acc_info.attr);
...
static int LSM6DS3_init_client(struct i2c_client *client, bool enable)
{
	...
	//obj->reso = &lsm6ds3_acc_data_resolution[0];
...
static int lsm6ds3_enable_nodata(int en)
{
	...
	else if(!pedo_enable_status && !tilt_enable_status && !smd_enable_status && !step_d_enable_status)
	{
...
static int lsm6ds3_step_c_enable_significant(int en)
{
	...
	if(1 == en)
	{
		...
		LSM6DS3_enable_pedo(priv->client, true);
	...
	else if(0 == en)
	{
		...
		{
			LSM6DS3_enable_pedo(priv->client, false);
		}
...
static int lsm6ds3_step_c_enable_nodata(int en)
{
	...
	if(value == 1)
	{
		...
		//step_notify(TYPE_STEP_COUNTER);/* for data init of step counter, add by liaoxl.st 8.17.2015 */
	}
	else
	{
		pedo_enable_status = false;
...
/* [liguanxiong start] */
static int get_acc_enable(void)
{
	u8 databuf[2] = {0};
	struct lsm6ds3_i2c_data *priv = obj_i2c_data;
	if (hwmsen_read_byte(priv->client, LSM6DS3_CTRL10_C, databuf)) {
		return 0;
	}
	if (databuf[0] & LSM6DS3_ACC_GYRO_FUNC_EN_MASK) {
		GSE_LOG("%s : acc already enable!\n",__FUNCTION__);
		return 1;
	}
	return 0;
}

static int get_pedo_enable(void)
{
	u8 databuf[2] = {0};
	struct lsm6ds3_i2c_data *priv = obj_i2c_data;
	if (hwmsen_read_byte(priv->client, LSM6DS3_TAP_CFG, databuf)) {
		return 0;
	}
	if (databuf[0] & LSM6DS3_PEDO_EN_MASK) {
		GSE_LOG("%s: pedo already enable!\n",__FUNCTION__);
		return 1;
	}
	return 0;
}
static int lsm6ds3_step_c_enable(int en)
{
	if (en == 1) {
		if (get_pedo_enable() == 0 || get_acc_enable() == 0) {
...
{
#if defined(LSM6DS3_STEP_DETECTOR)
	int res =0;
	struct lsm6ds3_i2c_data *priv = obj_i2c_data;
	...
	res = LSM6DS3_Enable_Step_Detector_On_Int(priv->client, en); // enable step detector interrupt
	if(res != LSM6DS3_SUCCESS)
	{
		GSE_LOG(" LSM6DS3_Enable_Step_Detector_On_Int failed!\n");
		return res;
	}
...
/*[wanglei start] Add lsm6ds3_step_c_enable_significant*/
static int lsm6ds3_step_c_enable_significant(int en)
{
    return 0;
}
...
/*[wanglei start] Add lsm6ds3_step_d_set_delay*/
...
/* [liguanxiong start]  */
#define PEDO_DATA_RESET_THRE	20000
#define EACH_PRINT_LOG_TIMES	1000;
static uint32_t pedo_count = 0;
...
static int lsm6ds3_step_c_get_data(uint32_t *value, int *status)
{
	...
	/* Need to reset PEDO DATA before it come to overflow */
	if (pedo_data > PEDO_DATA_RESET_THRE) {
		LSM6DS3_Reset_Pedo_Data(priv->client, LSM6DS3_ACC_GYRO_PEDO_RST_STEP_ENABLED);
		GSE_LOG("Reset_Pedo_Data, pedo_count = %d, pedo_data = %d\n", pedo_count, pedo_data);
		pedo_count += pedo_data;
		pedo_data = 0;
	}
	*value = pedo_count + pedo_data;
	*status = SENSOR_STATUS_ACCURACY_MEDIUM;
	if (--print_log_count < 0) {
		GSE_LOG("lsm6ds3_step_c_get_data = %d\n", *value);
		print_log_count = EACH_PRINT_LOG_TIMES;
	}
	return err;
}
/* [liguanxiong end]  */
...
#ifndef CONFIG_HAS_EARLYSUSPEND
/*----------------------------------------------------------------------------*/
/* [yanlin start] Used pm sleep mode */
//static int lsm6ds3_acc_suspend(struct i2c_client *client, pm_message_t msg)
static int lsm6ds3_acc_suspend(struct device *dev)
{
#if 1
...
//	{
		...
		if(pedo_enable_status || smd_enable_status || tilt_enable_status || step_d_enable_status)
		{
			return 0;
		}
...
#ifdef LSM6DS3_SIGNIFICANT_MOTION
		if (smd_enable_status) // the smd is one-shot sensor, so user need to re-enable it to get this event again while this event happened
		{
			smd_enable_status = false;
...
static int lsm6ds3_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	client->addr = LSM6DS3_I2C_SLAVE_ADDR;
	/* [liliwen add] Change i2c speed: 400kHz */
	//client->timing = 400;
	obj_i2c_data = obj;
	obj->client = client;
	...
	GSE_LOG("%s: OK\n", __func__);
	/* [liguanxiong start] Auto start step counter, and keep it is running */
	LSM6DS3_Reset_Pedo_Data(client, LSM6DS3_ACC_GYRO_PEDO_RST_STEP_ENABLED);
	lsm6ds3_step_c_enable_nodata(1);
	/* [liguanxiong end] */
...
static int lsm6ds3_step_c_local_init(void)
{
	...
	else
	{
		...
		/* [liguanxiong] Keep step counter always on */
		step_ctl.enable_nodata = lsm6ds3_step_c_enable;
		...
		/*[wanglei start] Add step_d_set_delay*/
...
MODULE_AUTHOR("xj.wang@mediatek.com, darren.han@st.com, jay.huangfu@st.com");
