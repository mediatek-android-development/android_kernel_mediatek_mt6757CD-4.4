...
/* [huangxiancong start] Add torch status control */
static int fl_set_timeout(int type_index, int ct_index, int part_index, int level)
{
	struct flashlight_operations *pf;
	struct flashlight_dev_arg fl_dev_arg;
	...
	fl_dev_arg.channel = fl_channel[type_index][ct_index][part_index];
	fl_dev_arg.arg = level;
	if (pf->flashlight_ioctl(FLASH_IOC_SET_TIME_OUT_TIME_MS, (unsigned long)&fl_dev_arg)) {
		fl_err("Failed to set timeout.\n");
		return -1;
	}

	/* set scenario to decouple mode */
	fl_dev_arg.arg = FLASHLIGHT_SCENARIO_DECOUPLE;
	if (pf->flashlight_ioctl(FLASH_IOC_SET_SCENARIO, (unsigned long)&fl_dev_arg)) {
		fl_err("Failed to set scenario.\n");
		return -1;
	}
...
static ssize_t set_torch_status(struct device *dev, struct device_attribute *attr, const char *buf, size_t size)
{
	...
	if (buf != NULL && size != 0) {
		...
		if (val == 1) {
			/* Torch LED1 ON */
			fl_set_timeout(0,0,0,0);
			fl_set_timeout(0,1,0,0);
			fl_set_level(0,0,0,7); //set to 200mA
			fl_enable(0,0,0,1);
			torch_led_status = 1;
		} else if (val == 2) {
			/* Torch LED2 ON */
			fl_set_timeout(0,0,0,0);
			fl_set_timeout(0,1,0,0);
			fl_set_level(0,1,0,7); //set to 200mA
			fl_enable(0,1,0,1);
			torch_led_status = 1;
		} else if (val == 3) {
			/* Torch LED1 ON */
			fl_set_timeout(0,0,0,0);
			fl_set_timeout(0,1,0,0);
			fl_set_level(0,0,0,3); //set to 100mA
			fl_enable(0,0,0,1);
			torch_led_status = 1;
		} else if (val == 4) {
			/* Torch LED1 ON */
			...
			fl_set_timeout(0,0,0,0);
			fl_set_timeout(0,1,0,0);
			fl_set_level(0,0,0,11); //set to 300mA
			fl_enable(0,0,0,1);
			torch_led_status = 1;
		} else {
			fl_enable(0,0,0,0);
...
/* [huangxiancong end] Add torch status control */
...
static int flashlight_probe(struct platform_device *dev)
{
	...
	/* [huangxiancong start] Add torch status control */
		...
		fl_err("Failed to create torch status file \n");
		goto err_create_torch_status_device_file;
        }
	/* [huangxiancong end] Add torch status control */
...
/* [huangxiancong start] Add torch status control */
err_create_torch_status_device_file:
...
{
	/* remove device file */
	sysfs_remove_file(kernel_kobj, &dev_attr_torch_status.attr); // [huangxiancong] Add torch status control
