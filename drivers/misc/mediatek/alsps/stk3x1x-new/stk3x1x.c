...
*******************************************************************************/
#ifdef STK_TUNE0
/* [yanlin start] modified calibration */
	#define STK_MAX_MIN_DIFF	200
//	#define STK_LT_N_CT	100
	#define STK_LT_N_CT	22
//	#define STK_HT_N_CT	150
	#define STK_HT_N_CT	40
	#define PS_THDH_LIMITED 1700
	#define PS_THDL_LIMITED 1600
	#define PS_MAX_THD_RANGE 1500
/* [yanlin end] modified calibration */
...
static int winfac = 155;
#else
static int winfac = 100;
#endif
// [wanglei end]
/*----------------------------------------------------------------------------*/
...
static int stk3x1x_enable_ps_set_thd(struct stk3x1x_priv *obj)
{
	...
	/* [yanlin start] modified calibration */
	else if (obj->ps <= PS_MAX_THD_RANGE)
	{
		atomic_set(&obj->ps_high_thd_val, obj->ps + obj->stk_ht_n_ct);
		atomic_set(&obj->ps_low_thd_val, obj->ps + obj->stk_lt_n_ct);
	}
	...
	{
		atomic_set(&obj->ps_high_thd_val, PS_THDH_LIMITED);
		atomic_set(&obj->ps_low_thd_val, PS_THDL_LIMITED);
	}
	/* [yanlin end] */
	/*
	else
...
static int stk3x1x_enable_ps(struct i2c_client *client, int enable, int validate_reg)
{
	...
	if(enable)
	{
		...
		if ((err = stk3x1x_write_ps_low_thd(obj->client, 0x0)))
		{
			APS_ERR("write low thd error: %d\n", err);
			return err;
		}
			...
			//	err = (obj->ps >= atomic_read(&obj->ps_high_thd_val))?0:1;
				...
				else if(stk3x1x_obj->hw->polling_mode_ps == 0)
				{
					...
					APS_LOG("%s:ps raw 0x%x -> value 0x%x \n",__FUNCTION__, obj->ps,
									distance_flag);
					/* [yanlin start] modified calibration */
					if (obj->ps < atomic_read(&obj->ps_low_thd_val))
						stk_ps_report(obj, 1);
					else if (obj->ps > atomic_read(&obj->ps_high_thd_val))
						stk_ps_report(obj, 0);
					/* [yanlin end] */
				}
...
err_out:
	/* [yanlin start] fix irq invalid when use calibration in Engineer mode */
/*
#if defined(CONFIG_OF)
	disable_irq(obj->irq);
#elif ((defined MT6573) || (defined MT6575) || (defined MT6577) || (defined MT6589) || (defined MT6572))
...
*/
	...
	enable_irq(obj->irq);
#endif
	/* [yanlin end] fix irq invalid when use calibration in Engineer mode */
...
static int stk_ps_tune_zero_func_fae(struct stk3x1x_priv *obj)
{
	...
	if(diff > obj->stk_max_min_diff)
	{
		...
		/* [yanlin start] modified calibration */
		if (obj->ps - obj->psi >= obj->stk_max_min_diff)
		{
			APS_LOG("--- %s: update threshold: ps=%d,psi=%d ---\n", __func__, obj->ps, obj->psi);
			if (obj->ps_high_thd_boot <= obj->psi + obj->stk_ht_n_ct)
			{
				atomic_set(&obj->ps_high_thd_val, obj->ps_high_thd_boot);
				atomic_set(&obj->ps_low_thd_val, obj->ps_low_thd_boot);
			}
		...
		else if (obj->psa - obj->ps >= obj->stk_max_min_diff)
		{
			APS_LOG("--- %s: update threshold: ps=%d,psa=%d ---\n", __func__, obj->ps, obj->psa);
			atomic_set(&obj->ps_high_thd_val, obj->psi + obj->stk_ht_n_ct);
			atomic_set(&obj->ps_low_thd_val, obj->psi + obj->stk_lt_n_ct);
		}
...
static void stk3x1x_eint_work(struct work_struct *work)
{
	...
	if(((1<<STK_BIT_PS) &  obj->pending_intr) && (obj->hw->polling_mode_ps == 0))
	{
		...
		/* [yanlin start] check if eint was error */
		if ((!(atomic_read(&obj->state_val) & STK_STATE_EN_PS_MASK))
			|| ((obj->ps < atomic_read(&obj->ps_low_thd_val)) && (distance_flag == 0))
			|| ((obj->ps > atomic_read(&obj->ps_high_thd_val)) && (distance_flag == 1)))
		{
			APS_ERR("--- stk ps eint error: state=0x%x ---\n", atomic_read(&obj->state_val));
...
/* eint request */
	if (stk3x1x_obj->irq_node) {
		...
		//enable_irq(stk3x1x_obj->irq);  /* [yanlin] fix irq invalid when use calibration in Engineer mode */
	} else {
...
static ssize_t stk3x1x_show_ps(struct device_driver *ddri, char *buf)
{
	...
	{
		/* [liguanxiong] ps data output format same as ltr579  */
		return scnprintf(buf, PAGE_SIZE, "0x%04X(%d)\n", stk3x1x_obj->ps, stk3x1x_obj->ps);
	}
}
/*----------------------------------------------------------------------------*/
...
static ssize_t stk3x1x_show_hw_info(struct device_driver *ddri, char *buf)
{
	return sprintf(buf,"type:\t%s\nvendor:\t%s\ndriver_version:\t%s\n",
			"STK3321-35A", "SensorTek", DRIVER_VERSION);
}
...
// [wanglei start]
// Add for als adjust
static ssize_t stk3x1x_show_winfac(struct device_driver *ddri, char *buf)
{
	return snprintf(buf, PAGE_SIZE, "%d\n", winfac);
}
static ssize_t stk3x1x_store_winfac(struct device_driver *ddri, const char *buf, size_t count)
{
	int value = 0;
	int ret = 0;
...
static DRIVER_ATTR(winfac,  S_IWUSR | S_IRUGO, stk3x1x_show_winfac,	stk3x1x_store_winfac);
// [wanglei end]
...
/* [yanlin add] */
static DRIVER_ATTR(hw_info, S_IWUSR | S_IRUGO, stk3x1x_show_hw_info, NULL);
/*----------------------------------------------------------------------------*/
static struct driver_attribute *stk3x1x_attr_list[] = {
    ...
    &driver_attr_hw_info,
    &driver_attr_winfac,  // [wanglei] Add for als adjust
};
...
{
	const char *name = "mediatek,stk3x1x";	// "mediatek,arg_stk3x1x";
