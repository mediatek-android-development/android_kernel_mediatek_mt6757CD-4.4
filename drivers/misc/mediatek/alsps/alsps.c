...
static void als_work_func(struct work_struct *work)
{
	...
	if (true ==  cxt->is_als_first_data_after_enable) {
		...
		/* [yanlin start] check the first data */
		if (value == 0 || value >= 20000) {
			ALSPS_LOG("--- may read abnormal data1:%d\n", value);
			if (value == 0) {
				if (++zero_count == 2) {
					zero_count = 0;
					cxt->is_als_first_data_after_enable = false;
			...
			if (true == cxt->is_als_polling_run)
				mod_timer(&cxt->timer_als, jiffies + 200/(1000/HZ));
			return;
		}
		...
		if (value == 0 || value >= 20000) {
			ALSPS_LOG("--- may read abnormal data2:%d\n", value);
			if (true == cxt->is_als_polling_run)
				mod_timer(&cxt->timer_als, jiffies + 200/(1000/HZ));
			return;
		}
...
/* [yanlin start] add tag and clear count */
als_report:
	zero_count = 0;
/* [yanlin end] */
...
als_loop:
	if (true == cxt->is_als_polling_run)
		...
		ALSPS_LOG("cxt->is_als_polling_run is false!\n");
}
...
static void ps_work_func(struct work_struct *work)
{
	...
	/* [yanlin start] return if not polling */
	if (cxt->ps_ctl.is_polling_mode == false) {
		ALSPS_ERR("ps_work_func: is not polling mode!!\n");
		return;
	}
	/* [yanlin end] */
	...
	if (1 == enable) {
		...
		if (false == cxt->is_als_polling_run && cxt->is_als_batch_enable == false) {
			if (false == cxt->als_ctl.is_report_input_direct) {
				...
				ALSPS_LOG("ALSPS enable, mod_timer\n");
			}
		}
	}
	...
	if (0 == enable) {
		...
		if (true == cxt->is_als_polling_run) {
			if (false == cxt->als_ctl.is_report_input_direct) {
				...
				ALSPS_LOG("ALSPS disable, del_timer\n");
			}
		}
		als_real_enable(enable);
	}
