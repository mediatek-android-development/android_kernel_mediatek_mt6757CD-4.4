...
static int bmi160_step_d_set_delay(u64 ns)
{
	return 0;
}
...
static int bmi160_acc_get_mode(struct i2c_client *client, u8 *mode)
{
	int comres = 0;
	u8 v_data_u8r = 0;
...
/* [liguanxiong start] Overwrite the enable function */
static int bmi160_step_c_enable(int en)
{
	int err = 0;
	u8 acc_mode = 0;
	u8 stc_enable = 0;
	struct step_c_i2c_data *obj = obj_i2c_data;
	if (en == 1) {
		err = bmi160_acc_get_mode(obj->client, &acc_mode);
		if (err < 0){
		...
		if (acc_mode == 0 || stc_enable == 0) {
			bmi160_step_c_enable_nodata(1);
		}
	}
	return err;
}
...
/* [liguanxiong start] */
#define CMD_STEP_COUNTER_RESET		0xB2
static int BMI160_STEP_COUNTER_RESET(struct i2c_client *client)
{
	u8 databuf[2] = {0};
	int res = 0;
	struct step_c_i2c_data *obj = obj_i2c_data;
	...
	databuf[0] = CMD_STEP_COUNTER_RESET;

	res = stc_i2c_write_block(client,
			BMI160_CMD_COMMANDS__REG, &databuf[0], 1);
	if(res < 0)
	{
		STEP_C_ERR("reset step_counter failed, res = %d\n", res);
		mutex_unlock(&obj->lock);
		return -1;
	}
	bmi160_stc_delay(4);
...
#define STEP_COUNTER_RESET_THRE		20000
#define EACH_PRINT_LOG_TIMES		1000;
static int valid_data_after_reset = 0;
...
static int bmi160_step_c_get_data(uint32_t *value, int *status)
{
	...
	if (err < 0) {
		STEP_C_ERR("read step count failed.\n");
		return err;
	}
	/* sometimes step count = 0 */
	if(0 == step_counter_val && valid_data_after_reset == 1) {
		valid_data_after_reset = 0;
		err = bmi160_acc_get_mode(obj->client, &acc_mode);
		if (err < 0){
			STEP_C_ERR("bmi160_acc_get_mode failed.\n");
		}
		...
		if(acc_mode != 0 && stc_enable == 1){
			step_counter_val = last_stc_value;
		}
	}
	if (step_counter_val != 0) {
		valid_data_after_reset = 1;
	}
	mdelay(30);
	if (step_counter_val >= last_stc_value) {
		obj->pedo_data.last_step_counter_value += (
				step_counter_val - last_stc_value);
		last_stc_value = step_counter_val;
	} else {
		last_stc_value = step_counter_val;
	}
	if (last_stc_value > STEP_COUNTER_RESET_THRE) {
		STEP_C_ERR("BMI160_STEP_COUNTER_RESET\n");
		err = BMI160_STEP_COUNTER_RESET(obj->client);
		if (err == 0) {
			last_stc_value = 0;
		}
	}
	*value = obj->pedo_data.last_step_counter_value;
	*status = 1;
...
static int step_c_i2c_probe(void)
{
	...
	ctl.enable_nodata = bmi160_step_c_enable;
	...
	ctl.step_d_set_delay = bmi160_step_d_set_delay;
	...
	step_c_init_flag = 0;
	STEP_C_LOG("%s: is ok.\n", __func__);
	/* [liguanxiong start] Auto start step counter, and keep it running */
	BMI160_STEP_COUNTER_RESET(obj->client);
	bmi160_step_c_enable_nodata(1);
	/* [liguanxiong end] */
	return 0;
