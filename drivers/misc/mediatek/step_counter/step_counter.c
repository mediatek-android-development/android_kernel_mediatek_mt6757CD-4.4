...
/* [liguanxiong start]  screen on/off notifier */
struct notifier_block step_counter_notifier;
static int is_screen_on = 1;
static int last_counter_value = -1;
static int step_counter_fb_notifier_callback(struct notifier_block *self,
...
/* [liguanxiong start] Modify screen off polling */
#define SCREEN_OFF_POLLING_DELAY	10000	// 10s
#define SCREEN_OFF_REPORT_TRIGGER_COUNTER	100
static void step_c_work_func(struct work_struct *work)
{
	...
	/* [liguanxiong] Do not report the value to user when screen off */
	if (is_screen_on == 1 && last_counter_value != cxt->drv_data.counter) {
		last_counter_value = cxt->drv_data.counter;
		step_c_data_report(cxt->idev, cxt->drv_data.counter, cxt->drv_data.status);
	} else if (is_screen_on == 0 && cxt->drv_data.counter > last_counter_value + SCREEN_OFF_REPORT_TRIGGER_COUNTER) {
		last_counter_value = cxt->drv_data.counter;
		step_c_data_report(cxt->idev, cxt->drv_data.counter, cxt->drv_data.status);
	}
	...
	if (true == cxt->is_polling_run) {
		{
			if (is_screen_on == 1) {
				...
				mod_timer(&cxt->timer, jiffies + SCREEN_OFF_POLLING_DELAY / (1000 / HZ));
			}
		}
	}
}
...
static int step_c_real_enable(int enable)
{
	...
	if (0 == enable) {
		/* [liguanxiong] Reset last_counter_value */
...
static int step_c_probe(void)
{
	...
	/* [liguanxiong start] Register screen on/off notifier */
	step_counter_notifier.notifier_call = step_counter_fb_notifier_callback;
	err = fb_register_client(&step_counter_notifier);
	if (err) {
		STEP_C_ERR("unable to register step_counter_notifier!\n");
	}
	/* [liguanxiong end] */
...
static int step_c_remove(void)
{
	...
	misc_deregister(&step_c_context_obj->mdev);
	if (err)
