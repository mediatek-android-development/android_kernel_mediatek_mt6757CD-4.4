...
#ifdef VANZO_LCD_RESUME_QUICK
static void mxc400x_enable(int en)
{
    printk("kls mxc400x_enable en = %d\n",en);
    int res =0;
    bool power = false;
    ...
    printk("kls MXC400X_enable_nodata OK en = %d sensor_power = %d\n", en, sensor_power);
    enable_status = en;
    mutex_unlock(&mxc400x_mutex);

}

static int handle_acc_event(void *unused)
{
    ...
    do
    {
        ...
        wait_event_interruptible(acc_waiter, acc_flag != 0);
        printk("kls acc is wake isen = %d\n",isen);
        mxc400x_enable(isen);
        acc_flag = 0;
...
{
#ifdef VANZO_LCD_RESUME_QUICK
    printk("kls mxc400x_enable_nodata en = %d\n",en);
    isen = en;
    printk("kls mxc400x_enable_nodata isen = %d\n",isen);
    acc_flag = 1;
...
#ifdef VANZO_LCD_RESUME_QUICK
        int acc_thread = 0;
        acc_thread = kthread_run(handle_acc_event,0,"acc_thread");
        if(IS_ERR(acc_thread))
        {
            err = PTR_ERR(acc_thread);
            printk("kls failed to create kernel thread :%d\n",err);
        }
#endif
...
/* @hyperion70	 misc_deregister(&mxc400x_device);
