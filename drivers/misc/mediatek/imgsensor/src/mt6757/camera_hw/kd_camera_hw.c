...
PowerCust PowerCustList = {
	{
		...
		{GPIO_UNSUPPORTED, GPIO_MODE_GPIO, Vol_High},	/* for MAIN2_DVDD; */ // [huangxiancong] change to use ldo
		...
		{SENSOR_DRVNAME_IMX386_MIPI_RAW,
			{
				...
				{DVDD, Vol_1100, 0},
				{AFVDD, Vol_2800, 0},
				//{PDN, Vol_Low, 0}, // [huangxiancong] Remove the unuse PDN
				//{PDN, Vol_High, 0},// [huangxiancong] Remove the unuse PDN
...
#endif
#if defined(SENSOR_DRVNAME_IMX338_MIPI_RAW)
...
#endif
#if defined(SENSOR_DRVNAME_S5K3P3SX_MIPI_RAW)
...
#endif
#if defined(SENSOR_DRVNAME_S5K5E2YA_MIPI_RAW)
...
#endif
#if defined(SENSOR_DRVNAME_S5K2P8_MIPI_RAW)
...
#endif
#if defined(SENSOR_DRVNAME_IMX258_MIPI_RAW)
		...
		{SENSOR_DRVNAME_OV8856_MIPI_RAW,
			{
				...
				{AVDD, Vol_2800, 0},
				{DVDD, Vol_1200, 0},
				//{AFVDD, Vol_2800, 1}, // [huangxiancong] Remove the unuse AFVDD
				{PDN, Vol_Low, 1},
				{PDN, Vol_High, 15},
				//{RST, Vol_Low, 0}, // [huangxiancong] Remove the unuse RST
				//{RST, Vol_High, 5},// [huangxiancong] Remove the unuse RST
				{VDD_None, Vol_High, 0}
			},
		},
#endif
...
#endif
#if defined(SENSOR_DRVNAME_S5K2X8_MIPI_RAW)
...
#endif
#if defined(SENSOR_DRVNAME_IMX214_MIPI)
...
#endif
#if defined(SENSOR_DRVNAME_IMX214_MIPI_MONO)
...
#endif
#if defined(SENSOR_DRVNAME_IMX230_MIPI_RAW)
...
#endif
#if defined(SENSOR_DRVNAME_S5K3L8_MIPI_RAW)
...
#endif
#if defined(SENSOR_DRVNAME_S5K2L7_MIPI_RAW)
...
#endif
#if defined(SENSOR_DRVNAME_IMX318_MIPI_RAW)
...
int mtkcam_gpio_init(struct platform_device *pdev)
{
	...
	has_mipi_switch = 1; // [huangxiancong] force to use mipi switch
...
/* [huangxiancong start] Add avdd dvdd pin control */
#define MAX_CAM_NUM     3
int cam_avdd_pin[MAX_CAM_NUM] = {GPIO27 | 0x80000000, GPIO28 | 0x80000000, GPIO28 | 0x80000000};
int cam_dvdd_pin[MAX_CAM_NUM] = {GPIO26 | 0x80000000, GPIO26 | 0x80000000, GPIO26 | 0x80000000};

BOOL cam_avdd_pin_enable(int camIdx, int status)
{
	if (camIdx >= MAX_CAM_NUM)
		return TRUE;

	if (mt_set_gpio_mode(cam_avdd_pin[camIdx],GPIO_MODE_00)) {
		PK_DBG("[CAMERA SENSOR] set gpio mode failed!! (AVDD%d)\n", camIdx);
		return FALSE;
	}

	if (mt_set_gpio_dir(cam_avdd_pin[camIdx], GPIO_DIR_OUT)) {
		PK_DBG("[CAMERA SENSOR] set gpio dir failed!! (AVDD%d)\n", camIdx);
		return FALSE;
	}

	if (mt_set_gpio_out(cam_avdd_pin[camIdx], status)) {
		PK_DBG("[CAMERA SENSOR] set gpio failed!! (AVDD%d)\n", camIdx);
		return FALSE;
	}
...
BOOL cam_dvdd_pin_enable(int camIdx, int status)
{
	if (camIdx >= MAX_CAM_NUM)
		return TRUE;

	if (mt_set_gpio_mode(cam_dvdd_pin[camIdx],GPIO_MODE_00)) {
		PK_DBG("[CAMERA SENSOR] set gpio mode failed!! (DVDD%d)\n", camIdx);
		return FALSE;
	}

	if (mt_set_gpio_dir(cam_dvdd_pin[camIdx], GPIO_DIR_OUT)) {
		PK_DBG("[CAMERA SENSOR] set gpio dir failed!! (DVDD%d)\n", camIdx);
		return FALSE;
	}

	if (mt_set_gpio_out(cam_dvdd_pin[camIdx], status)) {
		PK_DBG("[CAMERA SENSOR] set gpio failed!! (DVDD%d)\n", camIdx);
		return FALSE;
	}

	return TRUE;
}
/* [huangxiancong end] Add avdd dvdd pin control */
	...
	if (pwInfo.PowerType == AVDD) {
		if (pinSetIdx == 2) {
			if (PowerCustList.PowerCustInfo[CUST_MAIN2_AVDD].Gpio_Pin ==
			    GPIO_UNSUPPORTED) {
				#if 0 // [huangxiancong] AVDD use VSYS ldo
				if (_hwPowerOn(MAIN2_AVDD, pwInfo.Voltage) != TRUE) {
					PK_ERR("[CAMERA SENSOR] Fail to enable analog power2\n");
					return FALSE;
				}
				#endif
				/* [huangxiancong start] Add avdd pin control */
				if (TRUE != cam_avdd_pin_enable(pinSetIdx, GPIO_OUT_ONE)) {
					PK_ERR("[CAMERA SENSOR] Fail to power up AVDD_EN\n");
					return FALSE;
				}
				/* [huangxiancong end] Add avdd pin control */
			} else {
		...
		if (pinSetIdx == 2) {
...
#if 0 // [huangxiancong] Do not do this power change
			...
			if (PowerCustList.PowerCustInfo[CUST_SUB_DVDD].Gpio_Pin == GPIO_UNSUPPORTED) {
				 ...
				 * }
				 */
				#if 0 // [huangxiancong] Sub Camera DVDD use VIO28 ldo
				if (_hwPowerOn(SUB_DVDD, pwInfo.Voltage) != TRUE) {
					PK_ERR("[CAMERA SENSOR] Fail to enable digital power1\n");
					return FALSE;
				}
				#endif
				/* [huangxiancong start] Add dvdd pin control */
				if (TRUE != cam_dvdd_pin_enable(pinSetIdx, GPIO_OUT_ONE)) {
					PK_ERR("[CAMERA SENSOR] Fail to power up AVDD_EN\n");
					return FALSE;
				}
				/* [huangxiancong end] Add dvdd pin control */
			} else {
	...
	if (pwInfo.PowerType == AVDD) {
		if (pinSetIdx == 2) {
			if (PowerCustList.PowerCustInfo[CUST_MAIN2_AVDD].Gpio_Pin == GPIO_UNSUPPORTED) {
				...
				#endif
				/* [huangxiancong start] Add avdd pin control */
				if (TRUE != cam_avdd_pin_enable(pinSetIdx, GPIO_OUT_ZERO)) {
					PK_ERR("[CAMERA SENSOR] Fail to power down AVDD_EN\n");
					return FALSE;
				}
				/* [huangxiancong end] Add avdd pin control */
			} else {
		...
		if (pinSetIdx == 2) {
			...
			if (PowerCustList.PowerCustInfo[CUST_SUB_DVDD].Gpio_Pin == GPIO_UNSUPPORTED) {
				...
				#endif
				/* [huangxiancong start] Add dvdd pin control */
				if (TRUE != cam_dvdd_pin_enable(pinSetIdx, GPIO_OUT_ZERO)) {
					PK_ERR("[CAMERA SENSOR] Fail to power down DVDD_EN\n");
					return FALSE;
				}
				/* [huangxiancong end] Add dvdd pin control */
			} else {
		       ...
		       char *mode_name)
{
	...
	/* [huangxiancong start] Do not need to power other sensor if the Idx is wrong */
	if (currSensorName && (strcmp(currSensorName, SENSOR_DRVNAME_IMX386_MIPI_RAW) == 0)) {
		if (pinSetIdx != 0)
			goto _kdCISModulePowerOn_exit_;
	}

	if (currSensorName && (strcmp(currSensorName, SENSOR_DRVNAME_OV8856_MIPI_RAW) == 0)) {
		if (pinSetIdx != 1)
			goto _kdCISModulePowerOn_exit_;
	}

	if (currSensorName && (strcmp(currSensorName, SENSOR_DRVNAME_IMX386_MIPI_MONO) == 0)) {
		if (pinSetIdx != 2)
			goto _kdCISModulePowerOn_exit_;
	}
#if 0
...
#endif
	/* [huangxiancong end] Do not need to power other sensor if the Idx is wrong */
	...
	if (On) {
		...
		if (has_mipi_switch) {
			if (SensorIdx == DUAL_CAMERA_SUB_SENSOR) {
				//pinctrl_select_state(camctrl, cam_mipi_switch_en_l); // [huangxiancong] We do not have this pin
...
/* [huangxiancong] the mipi switch oe pin is always enable */
#if 0
		if (has_mipi_switch) {
			if ((SensorIdx == DUAL_CAMERA_SUB_SENSOR)
