...
static struct imgsensor_info_struct imgsensor_info = {
	...
	*/
	...
	/*stereo camera mono setting */
		...
		.linelength  = 4296,				/* record different mode's linelength */
		.framelength = 2011,			/* record different mode's framelength */
		.startx = 0,					/* record different mode's startx of grabwindow */
		.starty = 0,					/* record different mode's starty of grabwindow */
		.grabwindow_width  = 2016,		/* record different mode's width of grabwindow */
		.grabwindow_height = 1508,		/* record different mode's height of grabwindow */
...
static SENSOR_WINSIZE_INFO_STRUCT imgsensor_winsize_info[9] = {
	...
	{ 4032, 3016,	0,	0,	4032,	3016, 2016, 1508,	0,
		0, 2016, 1508, 0, 0, 2016, 1508}, /* normal video*/
	...
	{ 4032, 3016,	0,	380,	4032,	2256, 4032, 2256,   0,
		0, 4032, 2256,	 0, 0, 4032, 2256}, /* custom1 */
	{ 4032, 3016,	0,	0,	4032,	3016, 2016, 1508,   0,
		0, 2016, 1508,	 0, 0, 2016, 1508}, /* custom2 */
	{ 4032, 3016,	0,	0,	4032,	3016, 4032, 3016,   0,
		0, 4032, 3016,	 0, 0, 4032, 3016}, /* custom3 */
	{ 4032, 3016,	0,	0,	4032,	3016, 2016, 1508,   0,
		0, 2016, 1508,	 0, 0, 2016, 1508}, /* custom4 */
};
...
/* [huangxiancong start] Set Mono driver IC to standby mode */
static void write_af_reg(kal_uint8 addr, kal_uint8 reg, kal_uint16 para)
{
        char pusendcmd[4] = {(char)(addr & 0xFF), (char)(reg & 0xFF), (char)(para >> 8), (char)(para & 0xFF)};

        iWriteRegI2C(pusendcmd, 4, 0xEC);
}

static void set_af_standby(int status)
{
	if (status) {
		write_af_reg(0x84, 0x30, 0x0000); // Servo off
		write_af_reg(0x82, 0x59, 0x0000); // analog power off
		write_af_reg(0x82, 0x3D, 0x0000); // ADC disable
		write_af_reg(0x82, 0x72, 0x0000); // BTL disable
	} else {
		write_af_reg(0x82, 0x59, 0x000C); // analog power ON
		write_af_reg(0x82, 0x3D, 0x0080); // ADC Enable
		write_af_reg(0x82, 0x72, 0x1111); // BTL Enable
		write_af_reg(0x84, 0x30, 0x000D); // Servo ON
	}
}
/* [huangxiancong end] Set Mono driver IC to standby mode */
...
#if 0
	if (((imx386_primax_otp_buf !=NULL) && (imx386_primax_otp_buf[15] == 0x02)) ||
			((imx386_sunny_otp_buf !=NULL) && (imx386_sunny_otp_buf[15] == 0x02))) {
		write_cmos_sensor(0x6C00, 0x2C);
		write_cmos_sensor(0x6C01, 0x33);
		write_cmos_sensor(0x6C02, 0x13);
		write_cmos_sensor(0x6C03, 0x02);
		write_cmos_sensor(0x6C04, 0x50);
		write_cmos_sensor(0x6C05, 0x80);
	}
#endif
...
static void preview_setting(void)
{
	...
	/*pdaf setting*/
	write_cmos_sensor(0x3047, 0x00);/*PD_CAL_ENALBE*/
	write_cmos_sensor(0x3049, 0x00);/*PD_OUT_EN=1*/
	}
	...
	while (imgsensor_info.i2c_addr_table[i] != 0xff) {
		...
		do {
			*sensor_id = return_sensor_id() + 1;
			if (*sensor_id == 0x287) { // [huangxiancong] change mono sensor id to 387
				*sensor_id = IMX386_MONO_SENSOR_ID;
			}

			if ((ret >= 0) && (*sensor_id == imgsensor_info.sensor_id)) {
				LOG_INF("Get imx386 mono sensor success!i2c write id: 0x%x, sensor id: 0x%x, module_id:0x%x\n"
...
static kal_uint32 streaming_control(kal_bool enable)
{
	...
	if (enable) {
		set_af_standby(0); // [huangxiancong] Set Mono driver IC to active mode
		write_cmos_sensor(0x0100, 0X01);
	}
	else {
		write_cmos_sensor(0x0100, 0x00);
		set_af_standby(1); // [huangxiancong] Set Mono driver IC to standby mode
	}
	return ERROR_NONE;
}
#endif
