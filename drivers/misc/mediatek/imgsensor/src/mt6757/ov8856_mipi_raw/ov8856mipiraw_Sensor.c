...
static imgsensor_info_struct imgsensor_info = {
	...
	.pre = {
		.pclk = 147200000,				//record different mode's pclk
	...
	.margin = 8,			//sensor framelength & shutter margin//20160422  according to ov fae
...
/* [huangxiancong start] Add read and apply otp function */
#ifdef OV8856R2AOTP
struct otp_struct
{
...
static int RG_Ratio_Typical= 0x129 ; //297
static int BG_Ratio_Typical= 0x17A ; //378
static int apply_otp(struct otp_struct *otp_ptr)
{
	...
	if ((*otp_ptr).flag & 0x40)
	{
		...
		LOG_INF("apply OTP AWB successful. R_gain: 0x%x, B_gain: 0x%x, G_gain: 0x%x\n", R_gain, B_gain, G_gain);
	}

	//LOG_INF("R_gain: 0x%x, B_gain: 0x%x, G_gain0x%x\n",R_gain,B_gain,G_gain);
	...
	if ((*otp_ptr).flag & 0x10)
	{
		...
		LOG_INF("apply OTP Lenc successful\n");
	}
	return (*otp_ptr).flag;
}
#endif
/* [huangxiancong end] Add read and apply otp function */
...
static void sensor_init(void)
{
    ...
    */
	...
	write_cmos_sensor(0x370b, 0x7e); //20160422 according to ov Fae from 0x80to 0x7e for BLC performance
	...
	write_cmos_sensor(0x380e, 0x04);//04
	write_cmos_sensor(0x380f, 0xde);//de
...
static void preview_setting(void)
{
	...
	write_cmos_sensor(0x380c, 0x0e);
	write_cmos_sensor(0x380d, 0xa0);
	write_cmos_sensor(0x380e, 0x04); //0x04);
	write_cmos_sensor(0x380f, 0xfc); //0xfc);
	write_cmos_sensor(0x3810, 0x00);
...
#if 0 // [huangxiancong] do not need to check the eeprom version
...
#ifdef OV8856R2AOTP
				read_otp(&otp_ptr);
				if (otp_ptr.module_integrator_id != MODULE_SUNWIN_MID) {
					*sensor_id = 0xFFFFFFFF;
					LOG_INF("mid is wrong, expect is 0x%x, but read is 0x%x\n", MODULE_SUNWIN_MID, otp_ptr.module_integrator_id);
					return ERROR_SENSOR_CONNECT_FAIL;
				}
#endif
/* [huangxiancong end] Add read and apply otp function */
				front_camera_module_current = MODULE_OV8856_SUNWIN;
				return ERROR_NONE;
			}
