...
#include <cust_gyro.h>
#include "lsm6ds3-gyro.h"

#define POWER_NONE_MACRO MT65XX_POWER_NONE
#define LSM6DS3_GYRO_NEW_ARCH		//kk and L compatialbe

/*add for Gyroscope APK calibration 20150513 begin*/
#define LSM6DS3_MTK_CALIB                             //CALIB USED MTK fatctory MODE
//#define LSM6DS3_CALIB_HAL                             // CALIB FROM HAL
/*add for Gyroscope APK calibration 20150513 end*/
...
/*modify A+G sensro iic address 20150318 begin*/
...
/*add for selftest function 20150708 begin*/
...
#define MAX_ST  700
#endif
/*add for selftest function 20150708 end*/
...
/*modify fix gyro data vilation 20150608 begin*/
    s32                     data[LSM6DS3_GYRO_AXES_NUM+1];
/*modify fix gyro data vilation 20150608 end*/
	int 					sensitivity;
...
static int LSM6DS3_gyro_SetPowerMode(struct i2c_client *client, bool enable)
{
	...
	if(true == enable)
	{
		...
		/* [liliwen edit] change sample rate to 208Hz */
		databuf[0] |= LSM6DS3_GYRO_ODR_208HZ; //default set 100HZ for LSM6DS3 gyro
...
static ssize_t show_gyro_self_data(struct device_driver *ddri, char *buf)
{
	...
	/* [yanlin start] Check 0x00 RESERVED reg */
	databuf[1] = 0xFF;
	databuf[0] = 0x00;
	res = hwmsen_read_byte(client, databuf[0], &databuf[1]);
	if (res < 0)
	{
		GYRO_ERR("read RESERVED(0x%x) err!\n", databuf[0]);
		return LSM6DS3_ERR_I2C;
	}
	if (databuf[1] != 0x00)
	{
		GYRO_ERR("read RESERVED(0x%x)=0x%x!\n", databuf[0], databuf[1]);
		databuf[1] = 0x00;
		databuf[0] = 0x00;
		res = i2c_master_send(client, databuf, 0x2);
		if(res <= 0)
		{
			GYRO_ERR("write RESERVED(0x%x) err!\n", databuf[0]);
			return LSM6DS3_ERR_I2C;
		}
	}
	/* [yanlin end] Check 0x00 RESERVED reg */
...
static ssize_t show_board_info(struct device_driver *ddri, char *buf)
{
	return sprintf(buf,"type:\t%s\nvendor:\t%s\ndriver_version:\t%s\n",
		"LSM6DS3_GYRO", "ST", DRIVER_VERSION);
}
...
static int LSM6DS3_gyro_init_client(struct i2c_client *client, bool enable)
{
	struct lsm6ds3_gyro_i2c_data *obj = i2c_get_clientdata(client);
	int res = 0;
	u8 databuf[2]; // [yanlin] Check 0x00 RESERVED reg
	...
	//
	/* [liliwen edit] change sample rate to 208Hz */
	res = LSM6DS3_gyro_SetSampleRate(client, LSM6DS3_GYRO_ODR_208HZ);
	if(res != LSM6DS3_SUCCESS )
	{
		return res;
	}
...
/*modify A+G sensro iic address 20150417 begin*/
	client->addr = 0x6A;
/*modify A+G sensro iic address 20150417 end*/
