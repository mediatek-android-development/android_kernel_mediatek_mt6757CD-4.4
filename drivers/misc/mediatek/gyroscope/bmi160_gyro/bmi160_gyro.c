	...
	struct i2c_msg msgs[2] = {
		{
			...
			.len = 1,	.buf = &beg,
/* [liliwen edit] use client timing */
...
static int bmg_set_range(struct i2c_client *client, enum BMG_RANGE_ENUM range)
{
	...
	if (obj->sensor_type == BMI160_GYRO_TYPE) {/* BMI160_GYRO */
		...
		if (err < 0)
			...
			switch (range) {
			case BMG_RANGE_2000:
				obj->sensitivity = 1;//16;
			break;
			case BMG_RANGE_1000:
				obj->sensitivity = 2;//33;
			break;
			case BMG_RANGE_500:
				obj->sensitivity = 4;//66;
			break;
			case BMG_RANGE_250:
				obj->sensitivity = 8;//131;
			break;
			case BMG_RANGE_125:
				obj->sensitivity = 16;//262;
			break;
			default:
				obj->sensitivity = 1;//16;
			break;
		...
		int datarate)
{
	struct bmg_i2c_data *obj = obj_i2c_data;
	u8 err = 0, data = 0;
	/* [yanlin start] Fix data odr error */
	u8 rdata = 0;
	int i = 0;
	/* [yanlin end] */
	...
	if (obj->sensor_type == BMI160_GYRO_TYPE) {/* BMI160_GYRO */
		...
		/* [yanlin start] Fix data odr error */
		rdata = BMG_GET_BITSLICE(data, BMI160_USER_GYR_CONF_ODR);
		if (rdata != datarate) {
			GYRO_ERR("[%s] data = 0x%x, ODR change to 0x%x\n", __func__, data, datarate);
		} else {
			GYRO_ERR("[%s] Normal data = 0x%x, return now\n", __func__, data);
			mutex_unlock(&obj->lock);
			return 0;
		}
		...
		do {
			...
			mdelay(5);
			err += bmg_i2c_read_block(client,
				BMI160_USER_GYR_CONF_ODR__REG, &rdata, 1);
			GYRO_ERR("[%s] try %d = 0x%x\n", __func__, i++, rdata);
			rdata = BMG_GET_BITSLICE(data, BMI160_USER_GYR_CONF_ODR);
		} while ((i < 5) && (rdata != datarate));
		/* [yanlin end] */
	}
...
static int bmg_init_client(struct i2c_client *client, int reset_cali)
{
	...
	err = bmg_set_datarate(client, BMI160_GYRO_ODR_200HZ);
	if (err < 0) {
		...
		char *buf, int bufsize)
{
	...
	int err = 0;
	/* [yanlin start] Optimize calibration (2/3) */
	s32 datatmp[BMG_AXES_NUM];
	memset(datatmp, 0, sizeof(s32)*BMG_AXES_NUM);
	/* [yanlin end] */
	...
	if (err) {
		...
		/* [yanlin start] Optimize calibration (3/3) */
#if 1
		/* convert: LSB -> degree/second(o/s) */
		datatmp[BMG_AXIS_X] = (long) databuf[BMG_AXIS_X] / obj->sensitivity *  CORRECTION_FACTOR / DEGREE_TO_RAD_OLD;
		datatmp[BMG_AXIS_Y] = (long) databuf[BMG_AXIS_Y] / obj->sensitivity * CORRECTION_FACTOR / DEGREE_TO_RAD_OLD;
		datatmp[BMG_AXIS_Z] = (long) databuf[BMG_AXIS_Z] / obj->sensitivity * CORRECTION_FACTOR / DEGREE_TO_RAD_OLD;

		/* compensate data */
		datatmp[BMG_AXIS_X] += obj->cali_sw[BMG_AXIS_X];
		datatmp[BMG_AXIS_Y] += obj->cali_sw[BMG_AXIS_Y];
		datatmp[BMG_AXIS_Z] += obj->cali_sw[BMG_AXIS_Z];

		/* remap coordinate */
		gyro[obj->cvt.map[BMG_AXIS_X]] =
			obj->cvt.sign[BMG_AXIS_X] * datatmp[BMG_AXIS_X];
		gyro[obj->cvt.map[BMG_AXIS_Y]] =
			obj->cvt.sign[BMG_AXIS_Y] * datatmp[BMG_AXIS_Y];
		gyro[obj->cvt.map[BMG_AXIS_Z]] =
			obj->cvt.sign[BMG_AXIS_Z] * datatmp[BMG_AXIS_Z];
#else
		/* compensate data */
		databuf[BMG_AXIS_X] += obj->cali_sw[BMG_AXIS_X] * DEGREE_TO_RAD_OLD / DEGREE_TO_RAD;
		databuf[BMG_AXIS_Y] += obj->cali_sw[BMG_AXIS_Y] * DEGREE_TO_RAD_OLD / DEGREE_TO_RAD;
		databuf[BMG_AXIS_Z] += obj->cali_sw[BMG_AXIS_Z] * DEGREE_TO_RAD_OLD / DEGREE_TO_RAD;
		...
		/* convert: LSB -> degree/second(o/s) */
		gyro[BMG_AXIS_X] = gyro[BMG_AXIS_X] / obj->sensitivity * DEGREE_TO_RAD / DEGREE_TO_RAD_OLD;
		gyro[BMG_AXIS_Y] = gyro[BMG_AXIS_Y] / obj->sensitivity * DEGREE_TO_RAD / DEGREE_TO_RAD_OLD;
		gyro[BMG_AXIS_Z] = gyro[BMG_AXIS_Z] / obj->sensitivity * DEGREE_TO_RAD / DEGREE_TO_RAD_OLD;
#endif
		/* [yanlin end] */
...
/* [liliwen edit] Add gyro board info (1/3) */
#define DRIVER_VERSION    "1.0.0"
static ssize_t show_board_info(struct device_driver *ddri, char *buf)
{
	return sprintf(buf,"type:\t%s\nvendor:\t%s\ndriver_version:\t%s\n",
		"BMI160_GYRO", "BOSCH", DRIVER_VERSION);
}

/* [yanlin start] for hw info in /sys/kernel */
static ssize_t show_gyro_info(struct device *dev, struct device_attribute *attr, char *buf)
{
	return sprintf(buf,"type:\t%s\nvendor:\t%s\ndriver_version:\t%s\n",
		"BMI160_GYRO", "BOSCH", DRIVER_VERSION);
}
static DEVICE_ATTR(gyro_info, S_IWUSR | S_IRUGO, show_gyro_info, NULL);
/* [yanlin end] */
...
/* [liliwen edit] Add gyro board info (2/3) */
...
static struct driver_attribute *bmg_attr_list[] = {
	...
	/* [liliwen edit] Add gyro board info (3/3) */
...
static int bmg_create_attr(struct device_driver *driver)
{
	...
	err = sysfs_create_file(kernel_kobj, &dev_attr_gyro_info.attr);
...
static int bmg_delete_attr(struct device_driver *driver)
{
	...
	/* [yanlin add] for hw info in /sys/kernel */
	sysfs_remove_file(kernel_kobj, &dev_attr_gyro_info.attr);
...
#ifdef CONFIG_COMPAT
static long bmg_compat_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	long ret;
	...
	switch (cmd) {
		...
		case COMPAT_GYROSCOPE_IOCTL_SET_CALI:
			...
			GYRO_ERR("COMPAT_GYROSCOPE_IOCTL_SET_CALI\n");
...
static int bmi160_gyro_set_delay(u64 ns)
{
	...
	/* Currently, fix data rate to 100Hz. */
	int sample_delay = BMI160_GYRO_ODR_200HZ;  // [yanlin] Fix ODR to 200Hz
	struct bmg_i2c_data *priv = obj_i2c_data;
...
static int __init bmg_init(void)
{
	...
	if(!hw) {
		GYRO_ERR("Device tree configuration error!!\n");
		return 0;
	}
