...
/* [liliwen start] Add lcd info for TP908 */
#define PANEL_DRIVER_VERSION "1.0.0"
#define PANEL_FIRMWARE_VERSION "1.0.0"
#define PANEL_INFO_MAX  64

static ssize_t mtkfb_show_lcd_info(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	unsigned char vendor = 0;
	int ret = 0;

	unsigned char panel_ic[2][PANEL_INFO_MAX] = {{"ft8716"}, {"unknown"}};
	unsigned char panel_type[2][PANEL_INFO_MAX] = {{"tianma"}, {"unknown"}};
...
static DEVICE_ATTR(lcd_info, S_IRUGO, mtkfb_show_lcd_info, NULL);
/* [liliwen end] */
...
static int mtkfb_probe(struct platform_device *pdev)
{
	...
	/* [liliwen] Add lcd info */
	if (sysfs_create_file(kernel_kobj, &dev_attr_lcd_info.attr)) {
		DISPERR("sysfs_create_file lcd_info fail\n");
		return -ENODEV;
	}
