...
#define AF_DRVNAME "bu64748monoaf_DRV"
...
extern int BU64748_Initial_MAIN2(void);
extern void AF_TARGET_MAIN2(u16 target);
#ifdef CONFIG_MEIZU_BSP
extern int camera_main2_af_power(int on);
#endif
/*Add end */

int	SOutEx_MAIN2(u8 slaveAddress, u8 *dat, int size)
{
	int i4RetValue = 0;
...
int SInEx_MAIN2(u8 slaveAddress, u8 *dat, int size, u8 *ret, int ret_size)
{
	int ret_value = 0;
...
{
	return bu64748_main2_af_cur_pos(position);
}
#endif
/*Add end */
...
static inline int moveAF(unsigned long a_u4Position)
{
	...
	AF_TARGET_MAIN2(g_u4TargetPosition);
	spin_lock(g_pAF_SpinLock);
...
/* ////////////////////////////////////////////////////////////// */
long bu64748af_main2_Ioctl(struct file *a_pstFile, unsigned int a_u4Command, unsigned long a_u4Param)
{
	long i4RetValue = 0;
...
int bu64748af_main2_Release(struct inode *a_pstInode, struct file *a_pstFile)
{
	LOG_INF("Start\n");
	...
	if (*g_pAF_Opened == 2) {
		LOG_INF("Wait\n");
		msleep(20);
		I2C_func_POFF_____MAIN2(); //[huangxiancong] power off when close
	}
...
int bu64748af_main2_PowerDown(void)
{
	return 0;
}
...
/* Add by meizu BSP hudong@meizu.com */
#ifdef CONFIG_MEIZU_BSP
struct vcm_factory_fops bu64748_main2_fops = {
...
int bu64748af_main2_SetI2Cclient(struct i2c_client *pstAF_I2Cclient, spinlock_t *pAF_SpinLock, int *pAF_Opened)
{
	u8 out[4] = {0};
	u32 ret;
	...
	ret = SOutEx_MAIN2(_SLV_FBAF_, out, 4);
